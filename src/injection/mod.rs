...
fn set_clipboard_text(text: &str) -> Result<()> {
    ...
    unsafe {
        ...
        // CF_UNICODETEXT = 13; HGLOBAL and HANDLE both wrap isize
...
fn get_clipboard_text() -> Option<String> {
    unsafe {
        ...
        // HANDLE and HGLOBAL both wrap isize; transmute is safe here
...
fn send_ctrl_v() -> Result<()> {
    ...
    // Safety: inputs array is valid INPUT structs
...
fn inject_via_send_input(text: &str) -> Result<()> {
    ...
    if !inputs.is_empty() {
        // Safety: inputs vec contains valid INPUT structs
        unsafe { SendInput(&inputs, std::mem::size_of::<INPUT>() as i32) };
    }
    Ok(())
}
