...
**转出来的字是「听见的」，不是「能直接用的」。** 人说话天然带口水话、会改口、会重复，
照原样贴出去还得自己再收拾一遍。打开输出智能优化，这一步就省了：

| 它做什么 | 举个例子 |
|----------|---------|
| **去口水话** | 「嗯…那个…我觉得吧」→ 直接清掉 |
| **理顺改口** | 「这个方案，呃不对，是那个方案可行」→ 合成一句通顺的 |
| **纠同音错字** | 结合上下文和你的词库改回来 |
| **按场景排版** | 该分段就分段、该列表就列表、该一行到底就一行到底（见上文场景表） |
| **压掉啰嗦** | 正式写作场景里把重复表达的同一个意思合并；聊天场景不动 |

**它不会改你的意思。** 数字、单位、否定词、专有名词、文件路径、代码标识符都在保护范围里 ——
改的是表达方式，不是内容本身。
...
飞音的提示词是**针对 DeepSeek Flash（V4 / V4.1 Flash）专门调过的**，模型 ID `deepseek-flash`，
建议直接用 [DeepSeek 官方接口](https://deepseek.com)。

其它 OpenAI 兼容模型同样能跑，但「什么时候该列表、什么时候该保持一行」这类格式判断，
没有经过同等程度的打磨，效果会有出入。
