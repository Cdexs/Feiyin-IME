...
> **索引在 `troubleshooting.md`**（现象 → 判据）。本文件是全文，按 `[条目ID]` 搜索。
> 归档于 2026-09-10：原 troubleshooting.md 4814 行 ≈95K token，每次 session 启动全量加载，
> 与「启动必读合计 ≤20K token」约束冲突。**内容零删改**，只是搬了个地方。
> 🔴 新增条目：全文写进本文件，同时在 `troubleshooting.md` 索引里补一行。
> ⚠️ 本文件存在同一 ID 出现多次的历史条目（同一个坑的复发记录），搜索时注意看全部命中。
...
### 补充二（2026-08-03 19:00，orchestrator，coder-2）：✅ **`C-c` + `opencode -s <session>` 是有效解，且零上下文损失**

**这是上一条「待验证的处置手段」的实测结果 —— 有效，推荐为标准处置。**

- **现象**：与 coder-1 那次同形态 —— 三次 dispatch 全部 `ACK_FAIL`，屏幕停在启动态（还在显示「task.md 为空」和过期的 `790e316`/ahead 38），主控补发的短消息**输入框零痕迹**（不显示文本、不显示 QUEUED）。底部同样是**双模型名**（区块内 `kimi-k2.6` / 底栏 `kimi-k2.7-code Ollama Cloud`）——**双模型显示与冻结的相关性再次出现，两起两中**
- **判定**：间隔 15s 两次 `capture-pane | md5sum` **完全一致** → 冻结成立
- **处置阶梯（按此顺序，实测结论）**：
  | 手段 | 结果 |
  | --- | --- |
  | `send-keys Escape` | ❌ **无效**，md5 一字未变（与 coder-1 那次一致，Escape 对这个形态失效） |
  | `send-keys C-c` | ✅ **有反应** —— 但要有心理准备：**它会直接结束 OpenCode TUI**，pane 退回 bash 提示符 |
  | `opencode -s <session_id>` | ✅ **完整恢复，零上下文损失**（102.5K/39% 原样回来，会话历史在） |
- **🔑 关键发现**：OpenCode 被 `C-c` 结束时，会在退出画面上打印恢复命令：
  ```
  Continue  opencode -s ses_038d8436cffeC0N1ZssUXnZBi8
  ```
  **`C-c` 之后务必先 `capture-pane` 把这个 session id 抄下来再做别的**，用它重启即可带回全部上下文。这使得「杀掉 TUI」从破坏性操作降级为**低成本操作**
- **恢复后验证**：底栏模型名恢复为**单个** `kimi-k2.6 Ollama Cloud`（双模型消失）；发短消息后 12s 内屏幕出现 `$ echo ACK > ...` 与 `→ Read task.md`，ack 文件从 0 字节变 4 字节 → 输入通路确认恢复
- **决策口径（本次形成）**：Worker **有在途工作**时，优先按 coder-1 那次的做法「不动它、绕开」；Worker **无在途工作但在关键路径上**（本次 coder-2 要跑消融实验）时，直接走 `C-c` + `-s` 恢复，代价极低。**不要因为怕丢上下文而让关键路径干等**
- **仍未确证**：双模型名到底是 fallback 切换卡死的原因还是症状。两起均出现，但样本仅 2，且都在 Ollama Cloud 侧。下次再遇到时留意底栏模型名，攒到第三起再下结论

### 补充三（2026-08-03 19:2x，Gavin 告知根因）：🔴 **根因是「在 TUI 内手动切换模型」，已改为启动时定死**

**Gavin 原话**：「刚才 opencode 的 worker agent 卡死的原因是因为我手动去切换它的模型了。我发现**只要我在它的命令行控制台输入切换模型的指令的时候，它就会卡死**，切换到新模型就会卡死。」

**这条解释了补充一/补充二里那个「双模型名」现象**：底栏显示的是**要切换过去的新模型**，消息区块里还是**旧模型**，切换动作卡在中间没完成 —— 所以双模型名不是 fallback 的副作用，**是「切换进行中且已挂死」的直接显示**。四次观察（coder-1 两次、coder-2 一次、coder-1 复发一次）全部吻合。

**根治方式：模型在启动时用 `-m` 一次定死，运行期不切换。**

- OpenCode 1.15.3 支持 `-m, --model`，格式 `provider/model`；可用值 `opencode models`
- 本项目取值（provider 用 `ollama-cloud`，与底栏显示一致）：
  | Worker | 模型 |
  | --- | --- |
  | coder-1 | `ollama-cloud/glm-5.2` |
  | coder-2 | `ollama-cloud/kimi-k2.7-code` |
  | tester-1 | `ollama-cloud/kimi-k2.6` |

**框架改动（已落地，2026-08-03）**：

| 文件 | 改动 |
| --- | --- |
| `collab/configs/team-voiceime.yaml` | worker 条目新增可选字段 `model:`；三个 OpenCode Worker 各指定模型 |
| `collab/lib/config-parser.sh` | 新增 `CONFIG_WORKER_MODEL` 关联数组 + `get_worker_model()` + `build_worker_cmd()`；yq 路径与降级 bash 路径**都**解析 `model:`。**刻意不改 `id\|role\|type\|desc` 那条管道记录的格式**，避免影响既有消费方 |
| `collab/init-collab.sh` | `cmd=$(get_type_cmd ...)` 之后追加 `cmd=$(build_worker_cmd "$cmd" "$type" "$id")` |
| `collab/replace-worker.sh` | 替换 Worker 时沿用配置中的模型；配置路径从 `state.json` 的 `config_file` 取；支持第 3 参数显式覆盖 |

`build_worker_cmd` **只对 OpenCode 类型追加 `-m`**（其它 Agent 模型参数写法不同），未配置 `model` 时原样返回，故对 orchestrator(Claude) 无影响。实测四个 Worker 命令组装正确。

**顺带修掉的一个雷**：`team-voiceime.yaml` 里 `OpenCode:` 原先**顶格写**（其余四个类型都缩进 2 格）。本机 **yq 未安装**，走的是宽松的降级行解析器所以一直没出事；**一旦有人装了 yq，`.types.OpenCode.cmd` 会解析成 null，三个 OpenCode Worker 会全部以空命令启动**。已补上缩进。

**操作纪律（对所有人，含 Gavin）**：

- ❌ **不要在 OpenCode TUI 里手动切换模型** —— 必卡死
- ✅ 要换模型：改 `team-voiceime.yaml` 的 `model:` 字段，下次 `init-collab.sh` 生效
- ✅ 只想临时换当前会话的模型：`C-c` 退出 → 抄下 `Continue opencode -s ses_xxx` → 用 `opencode -s ses_xxx -m <provider/model>` 重启（**带 `-s` 保上下文，带 `-m` 定模型，全程不进 TUI 切换**）
...
## [F3-UNORDERED-LIST-001] 🔴 无序枚举不出列表：三轮消融实验、11 个假设全部证伪，机制未明【再碰此问题前必读】

**状态**：🟡 **部分解决 + 明确挂起**（2026-08-03，Gavin 拍板选项二收尾）
**成本**：约 210 次真实 API 调用，三轮实验（coder-2 一轮、tester-1 两轮）
**这条存在的意义**：**把已经排除掉的路全部记下来，让下一个人不用重走这 210 次。**

### 一、现象

Gavin 端测：说「比如…再比如…还有就是…」这类无序枚举，输出仍是一整段，不走 `- ` 列表。而「第一点…第二点…第三点」这类有序枚举**从来没失败过**。

### 二、🔴 已被实验证伪的 11 个假设（不要再试）

**提示词侧（coder-2 阶段一，6 组 × 3 输入 × 3 次）**

| # | 假设 | 变体 | 结果 |
| --- | --- | --- | --- |
| 1 | 018 分层把 F3 架空（`output_contract` 在 L1、F3 在 L3，元规则宣布 L3 必输） | V0 基线 | 对 IN-B/C 无效 |
| 2 | L1 的「默认单段落」表述压制了列表 | V1 删该表述 | 无效 |
| 3 | F3 层号太低，提到 L1 就好 | V2 | 无效 |
| 4 | 上面两条叠加 | V3 | 无效 |
| 5 | 012 的「合并成一段放最末拿 recency」才是真解 | V4 复现 012 结构 | 无效（IN-C 仅 1/3） |
| 6 | F3 段开头虚假自称权威（`SINGLE authority on lists, line structure, and output tags`）制造歧义 | V6 | 无效 |

**提示词侧（tester-1 阶段二）**

| # | 假设 | 变体 | 结果 |
| --- | --- | --- | --- |
| 7 | 无序 few-shot 要求删标记词（`比如X` → `- X`），与 L0-1「每个语义单元必须保留」冲突；有序则不用删词（`1. 第一点X`） | V7 保留标记词 | IN-B 0/3、IN-C 1/3，**噪声级** |
| 8 | 「引入句 + 列表 + 收尾句」的混合结构从未被授权 | V8 显式授权 | **零效果** |

**输入侧（tester-1 阶段三，固定提示词只改输入）**

| # | 假设 | 变体 | 结果 |
| --- | --- | --- | --- |
| 9 | 末尾独立总结句（「上面的现象都是不好的…」）导致退回段落 | B1 删总结句 / C1' | **0/3，证伪** |
| 10 | 开头引入句框住了叙述语气 | B2 删引入句 / B3 两头都删只剩三项 | **0/3，证伪**（纯三项照样不出） |
| 11 | 口水词干扰 / 项目太长 | B5 删口水词 / B6 缩短项 | 均 0/3，证伪 |

### 三、唯一起作用的变量（实验确证）

**B4：同一句话、同一引入句、同一总结句、同一口水词、同一项目内容，只把三个无序标记（比如说/还有比如/比如还有）换成有序标记（第一/第二/第三）→ 立刻 3/3 出列表。**

### 四、结论与边界（**注意这里有一处对 tester-1 报告的订正**）

- tester-1 报告写的是「无序标记**在框架句内**被系统性漏识别」。**「框架句」这个限定词不成立** —— B3 已把框架全删、只剩三个光秃秃的并列项，仍然 0/3。
- 另一个反证：**IN-A（建议从以下方面入手：比如…再比如…还有比如就是…）用的也是无序标记，却 3/3 成功**。
- 因此准确表述是：**有序标记的识别是稳的；无序标记的识别处在临界点上** —— 有的内容能过（IN-A），有的过不去（IN-B/IN-C）。差别在**项目内容本身**，不在结构、不在框架、不在长度。
- **机制未明。** IN-A 的项目是并列的动作建议；IN-B/C 的项目是带内部顿号的描述性长句。是什么让后者判不出并列，**未定位**。

### 五、为什么在这里停（Gavin 2026-08-03 拍板）

1. **021 已修好最常见形态** —— 实验反复证实 IN-A 那类稳定出列表；本问题的影响面比最初判断的窄得多
2. **有序枚举全程稳定**，从未失败
3. **不知道机制就改提示词 = 打补丁**，是 Gavin 明令禁止的。tester-1 建议「新增框架句内无序枚举 few-shot」**未被采纳** —— B3 已证明框架句不是原因，照此加例子是在错误方向上加东西
4. 已知**可靠规避方式**：改用有序说法（第一/第二/第三）

### 六、下次要继续时，从这里接着做（唯一还没试过的切法）

范围已被 B3 锁死到「**IN-B 的三个项目本身，不带任何框架，就判不出枚举**」。下一步应做**项目内容侧**的二分，而不是再回头改提示词：

- **B8**：IN-B 三项缩短 + 无框架（B3 与 B6 叠加，两者单独都失败过，叠加未试）
- **B9**：IN-A 的项目 + IN-B 的框架 —— 若成功，则框架彻底无关，锁定为项目性质
- **B10**：把 IN-B 三项改写成无内部顿号的短句，其余不动

### 七、方法论沉淀（本次最值钱的部分）

- **主控在本问题上连错三次**：①静态取证推出一个"完全闭环"的根因（代码行号、元规则原文、012/018 因果链、用户原句逐字在 few-shot 里）—— 实测一枪打空；②统计时读了 Worker 的临时中间文件，下了"六组全灭"的错误结论 —— 被 coder-2 顶回；③拿 IN-A 的实验结果去比 IN-B 的生产结果（输入不同、提示词版本也不同），凭空造出一个"1480 token 缺口"，害 tester-1 白跑两轮 —— 被 tester-1 顶回。
- **三次都是同一个毛病：把"看起来能解释"当成"已经验证"。**
- **提示词类问题的因果链，只能由真实 API 消融实验确认，静态阅读再严密也只是假设。** 这条纪律本次直接避免了一次无效的架构改动（V3 就是主控原本要直接落地的方案，实测零效果）。
- **实验设计要能翻身**：前两轮都是「固定输入改提示词」，全空；第三轮换成「固定提示词改输入」，一轮就锁定了唯一变量。**同一设计连续失败两轮，应换设计而不是加变体。**

### 八、2026-08-14 复现（BUILD-015 端测）：**排除 030 提示词改动，老问题原样复发**

**Gavin 提问**：「是不是之前我们改动了 LLM 模型的提示词改坏了？」—— 结论 **不是**。

**新数据点（IN-D，此前未测过的输入）**，`target/release/debug.log:27`（05:41:45Z）：

```
输入：你可以去菜场看一看有哪些新鲜的水果和蔬菜，比如有没有新鲜的青椒，
      有没有新鲜的西瓜，还有没有比较新鲜的香蕉，这些啊你都可以看一看，有合适的就买。
输出：（一整段，仅删掉两个「啊」，零列表）
```

**逐条取证（全部来自同一次请求的日志，非推测）**：

| # | 检查项 | 实测 | 说明 |
| --- | --- | --- | --- |
| ① | F4 无序枚举规则是否在场 | ✅ **在场且完整** | 日志 `Scene F4 block injected` 原文含 `比如/还有/另外/此外`、`use "- " for unordered enumeration`、以及 DEC-039 语义并列兜底句 |
| ② | 是否被单行模式压制 | ❌ 不是 | `Notepad.exe`, `kind=document`, **`multiline_safe=true`** |
| ③ | 提示词是否完整送达 | ✅ | `system_prompt len=22055`，`prompt_tokens=5361` |
...
- `src/llm/mod.rs:1146` `f3_rules_text`：`true → INLINE_SEPARATOR_RULES`（**原常量**），030-C 的
  `INLINE_SEPARATOR_RULES_NO_PUNCT` 及买菜示例空格版仅在 false 分支
- `build_translate_system_content` 的 B4/B5 属**翻译路径**，本次 `translate=false` 未走

而本次输出**带完整中文标点且无空格分隔** → `punctuation_enabled = true` → 以上改动一处都没进提示词。

**顺带排除的第二嫌疑**：F4 文本源自 `scene-rules.toml:323`（document 场景），BUILD-015 曾首次把
macOS 端 `f96c817` 的新版同步进包。已 `git diff f96c817^ f96c817 -- scene-rules.toml` 核对，
**枚举段零改动**，该嫌疑排除。

**⚠️ 明确未证实的部分（避免重蹈第七节「把看起来能解释当成已验证」）**：
本条只证明了「不是 030 改的」，**没有证明** IN-D 与 IN-B/IN-C 是同一机制。IN-D 是全新输入，
且其项目形态与 IN-B/C 不同 —— IN-D 三项是**嵌在建议框架里的动词性疑问短语**
（`有没有新鲜的X`），不是 IN-B/C 那种带内部顿号的描述性长句，却同样失败。

**对第六节实验设计的增量价值**：IN-D 可直接充当 B10 的天然对照 ——
它的项目**本来就没有内部顿号、本来就短**，仍然 0/1。若该结果能稳定复现（需 3 次），
则 B10（「把 IN-B 三项改写成无内部顿号的短句」）大概率也会失败，可优先做 **B9**
（IN-A 项目 + IN-B 框架）来锁定「项目性质」这一变量。

### 🔴 关键更正：模型变量早已测过，且本条目此前**漏记了决定性的阶段六**

主控最初把「模型型号」列为未测盲区 —— **这是错的**。取证后发现
`collab/research/prompt-f3-model-probe-025f.md`（2026-08-03，tester-1 阶段六）**专门做过模型对照**：

| 输入 | `deepseek-v4-pro` | `deepseek-v4-flash`（当时生产） |
| --- | --- | --- |
| IN-B | **3/3 出列表** ✅ | 0/3 段落 ❌ |
| IN-C | **3/3 出列表** ✅ | 0/3 段落 ❌ |
| C2 单例（反向对照） | 3/3 段落 ✅ | — |
| C4 叙事无并列（反向对照） | 3/3 段落 ✅ | — |

当时结论：「**本问题是模型能力边界，不是提示词缺陷**」，建议方案 A —— 生产模型升 `deepseek-v4-pro`。
**Gavin 已采纳**：`target/release/config.toml:8` 现为 `model = "deepseek-v4-pro"`。

**⚠️ 本条目一~七节是阶段五（含）之前的状态，从未补记阶段六**，全文停留在「机制未明、挂起」，
导致 2026-08-14 主控读完整条后仍把模型判为盲区。**文档漏记直接造成了一次错误结论。**

### 🔴 因此本次复现的真正意义：**升级模型抬高了阈值，但没有消除它**

今天这次失败**就跑在 `deepseek-v4-pro` 上**（日志 `Request body: model=deepseek-v4-pro`）。
也就是说：

- 阶段六证明 v4-pro 能吃下 IN-B / IN-C —— **这个结论仍然成立，没被推翻**
- 但 IN-D（本次买菜句）是 **v4-pro 上的全新失败样本** —— 说明 v4-pro **不是完全解**

「模型能力边界」的判断方向是对的，但**边界的位置比阶段六以为的更靠前**。
换模型把一部分样本从失败区搬进了成功区，IN-D 说明失败区仍然存在。
...
**状态**：🟡 模式已识别，实例逐个收口中（2026-08-04 起）
**来源**：Gavin 2026-08-04 指令「要分析此类问题的根源，**从机制上解决**，不能再犯类似问题」

### 一、模式定义

> **一条为短表达设计的特例规则，没有声明自己的适用边界，于是在更长的上下文里继续生效，把正确的解析改坏。**

这不是某一个 bug，是**同一个病的反复发作**。四个月里至少四次，每次都是 Gavin 端测撞出来的：

| 编号 | 规则本意（正确） | 越界后果 | 发现方式 |
| --- | --- | --- | --- |
| **016** | 逐位串「一三」=13（电话号/房间号） | `一三班`（一年级三班）→ `13班` | Gavin 端测 |
| **026** | 隐式末级尾数（`五块一`→5.1元） | `after_is_boundary` 与数值合成耦合 → `五块一斤` 撕裂成 `5块`+`一斤` | Gavin 端测 |
| **027-A** | 隐式千位（`两万五`=25000） | 万后走完完整进位链时，末尾单字仍被 +1000 → `一千零四十六万八千七百四十一` 差 999 | Gavin 端测 |
| **027-C** | `两` 兼作重量单位（017 为 `一斤二两` 引入） | `is_unit` 的 `starts_with` 使「亿**两**千…」被判为「亿后跟单位」→ **整串金额蒸发成 `1`** | 主控复查（Gavin 提新需求倒逼） |

### 二、为什么这类缺陷特别难发现

1. **短表达全对**：规则在设计场景里 100% 正确，单测也是照设计场景写的
2. **需要足够长的上下文才暴露**：`两万五` 对、`三万五千二百四十一` 错——差别只在多了几个字
3. **常有一个"歪打正着"的掩护条件**：027-A 里凡带「零」的（`三万五千二百零一`）因 `zero_since_big=true` 走 else 分支反而正确；027-B 里单独「N万」时 `result==0` 使错误公式恰好等价。**掩护条件让缺陷在最常见的用例上不显形**
4. **静默改错**：数字类缺陷输出流畅自信，用户看不见（第四种失败模式，见 `[BUGREPORT-SELFCORRUPT-001]` 与 017）

### 三、机制性防御（2026-08-04 确立）

> **任何「隐式/省略成分补全」类规则，必须显式声明适用边界，并配反向护栏测试证明它在边界外不生效。**

落地要求：

1. **代码注释必须写清三件事**：规则适用什么、**不适用什么**、边界判据是什么。参照 `src/itn.rs` 027-A 修复后的注释格式（① `big_unit_seen` ② `!zero_since_big` ③ `!unit_since_big` 三条件逐条说明）
2. **成对测试**：每条补全规则同时要有「适用生效」与「边界外不生效」两侧断言。只测生效侧 = 没测
3. **收窄优先于新增路径**：027-A/B 的修法都是「给已有分支加一个条件」，不新增代码路径——这样零回归验证的判别力最强
4. **改动前存基线，改动后逐条比对**（Gavin 一票否决门禁）

### 四、已盘点、尚未收口的同模式规则（coder-1 2026-08-04 交付，**只列未改**）

| # | 位置 | 未声明的适用边界 | 状态 |
| --- | --- | --- | --- |
| 1 | `src/itn.rs:805` 亿级隐式单位 | 注释称「三亿五=350000000（五→五千万）」，代码乘 **1000** → 实测 `300005000`。**注释与实现不符** | ⏸ 待 Gavin 拍板改哪边（口语「三亿五」通常指 3.5 亿） |
| 2 | `src/itn.rs:699/716` `large_amount_keep_wan_yi` | `is_unit` 用 `starts_with`，「两」既是数字又是单位 | 🔄 027-C 进行中 |
| 3 | `src/itn.rs:682` 十分支 `(if has_digit { digit } else { 1 }) * 10` | 「十」无前导数字时默认 1，大单位后 `零十万` 应为 10 还是 0 未声明 | ⏸ 待实测确认是否真缺陷 |
| 4 | `src/itn.rs:900` 丙型 UnitChain 调 `parse_cn_number` | 丙型只取 section 级值，parse 遇大单位结算到 `result` 后**被丢弃**，多段链含万/亿可能漏算 | 🔄 已挂 027-C 附加实测 |

### 五、与既有条目的关系

- `[ITN-PREFIX-SHADOW-001]`：那条讲的是**词表**的前缀遮蔽（`is_unit` 用 `starts_with` + 中文无词边界）。**027-C 是同一个根**长在规则层的分支——`starts_with` 不只遮蔽词表匹配，还会让「万/亿后跟单位」这类结构判据误判。
- `DEC-038`：保护词表不得承载规则性语法族。本条是它的**对偶**——规则层的特例也不得无边界扩张。两条合起来才完整：**词表管不可推导的，规则管可推导的，各自都要声明边界。**

### 六、教训归属

四次全部由 Gavin 端测（或端测引出的需求）发现，**没有一次是测试先抓到的**。原因是单测按设计场景写，而缺陷只在设计场景之外显形。第三节第 2 条（成对测试，必须有边界外断言）就是针对这一点。

---

## [ENCODING-UTF8-001] 补充（2026-08-04）：Python 写文件同样会 GBK 截断，且「产物早于源文件」不等于产物是旧的

### 一、事故：Python 写 `src/itn.rs` 触发 GBK 截断

**发生**：2026-08-04 tester-1 落地 S1-S5 补测时，用 Python 写 `src/itn.rs` 触发 GBK 编码截断，文件被破坏。tester-1 自行恢复并重跑确认，**主动向主控申报**（未隐瞒，这一点做对了）。

**根因**：Windows 上 Python 的 `open()` 默认编码取 `locale.getpreferredencoding()`，中文 Windows 下即 **GBK**。写入含中文的 UTF-8 源文件时，遇到 GBK 无法编码的字符即抛异常或截断。

**这条此前只写了 PowerShell**（`Set-Content` / `Out-File` 默认 UTF-16 LE），实际 **Python 同样中招** —— 原条目「用 WSL Python 替代 PowerShell」的建议**不完整**：关键不在用哪个语言，而在**是否显式指定编码**。
...
# ❌ 依赖默认编码，Windows 中文环境 = GBK
open(path, 'w')
```

**WSL 内的 Python 默认 UTF-8（`LANG=C.UTF-8`），相对安全**；但 Windows 原生 Python 必须显式指定。**统一要求：任何语言写 UTF-8 源文件，一律显式声明编码，不依赖默认值。**

### 二、截断是否已恢复：四条独立判据（主控 2026-08-04 实用）

Worker 报「已恢复」时，**不要采信，用这四条独立验证**：

| # | 判据 | 本次实测 |
| --- | --- | --- |
| 1 | **`git diff -U0` 看有无大量删除行** | 只有 `@@ -4437,0 +4438,112 @@` 一个 hunk，**纯新增零删除** → 截断必然产生删除行，无删除即已恢复 |
| 2 | **`cargo check --tests` 编译** | `Finished`，零 error → 文件结构完整、语法正确 |
| 3 | **U+FFFD 替换字符计数** | 0 → 无解码损坏残留 |
| 4 | **文件末尾闭合** | 末 50 字符为 `...);\n    }\n}\n` → 大括号正常闭合，非中途截断 |

**判据 1 最快最灵**：截断的本质是内容丢失，在 git 里必然表现为删除行。一条 `git diff --stat` 即可初判。

### 三、🔴 「产物 mtime 早于源文件」不等于产物是旧的（本次新增判定方法）

**现象**：BUILD-014 出包后核对 mtime 链，发现产物**早于**源文件 3 分钟：

```
src/itn.rs        18:11:09    ← 比产物晚
Publish/*.exe     18:08:07
```

按 `[BUILD-002]` 的常规判据这是「产物是旧的」，应当重建。**但本次判定为实质安全，未重建。**

**判定方法（三步）**：

1. **查生产代码定型时间**：`git log -1 --format=%ad <commit>` → 027-F 提交于 **17:57:14**，早于构建 18:08 ✅
2. **查晚于产物的那次改动碰了什么**：`git diff -U0 <file> | grep '^@@'` → 起始行 **4438**，而 `mod tests` 起于该行之后 → **只碰了测试块**
3. **测试代码不进 release binary** → 产物内容与该次改动无关

**结论口径**：mtime 链**形式上不通过、实质安全**。如实标注，不要粉饰成「通过」，也不要机械重建浪费一轮构建。

**反过来说，什么情况必须重建**：晚于产物的改动落在**生产代码区间**（`mod tests` 之前）——那时产物确实是旧的，无论 Worker 怎么解释都要重建。

### 四、连带教训

- 事故发生在**出包之后、报告之前**，若 tester-1 不主动申报，主控只会看到「七判据全过」的漂亮报告。**Worker 主动申报事故的价值，高于报告好看**
- 但主控**不能依赖 Worker 的诚实**作为唯一防线 —— 第二节四条判据是低成本的独立验证，应固化为「Worker 报告编码事故后的标准验收动作」

---

## [REPLACE-WORKER-INJECT-LOST-001] 🔴 Worker 重启后上下文注入被静默吞掉，脚本仍报「完成」【重启 Worker 前必读】

**状态**：✅ **已修复**（2026-08-14，Gavin 指令修）
**发现方式**：2026-08-14 按 Gavin 指令给 coder-1/coder-2 换模型并重启，**两次注入全部丢失**，
主控 `capture-pane` 才发现两个 pane 停在零对话的空白欢迎页。

### 一、现象

`replace-worker.sh` 打印完整的六步成功流水（含「✅ OpenCode 就绪」「⑤ 注入上下文」
「✅ Worker 替换完成」），但 Worker 那边**一个字都没收到** —— 输入框仍显示占位符
`Ask anything...`，对话区空白，上下文 0 tokens。

**故障是完全静默的**：脚本零报错、零警告。若主控不主动 `capture-pane`，会直接向一个
「不知道自己是谁、不知道 inbox 在哪、不知道怎么回传 ACK」的 Worker 派发任务。
...
```bash
# 旧代码（replace-worker.sh:96 / init-collab.sh:398 / init-collab.sh:554 三处同样写法）
if echo "$pane_content" | grep -qiE 'Build|Ollama|kimi|glm|ctrl\+p|esc interrupt'; then
    ready=1   # ← 判定「可以打字了」
fi
```

`Build`、`Ollama`、`glm` 这些字样**在 OpenCode 的启动横幅阶段就已经渲染出来了**，
此时 TUI 的输入框尚未挂载、键盘输入无处可去。脚本据此判定就绪并立即 `send-keys`，
文本敲进一个还不接受输入的终端，**被直接丢弃**。

**第二层：注入之后零校验。**

```bash
# 旧代码
tmux send-keys -t "$P" -l "$INJECT_MSG" 2>/dev/null || true
sleep 0.2
tmux send-keys -t "$P" Enter 2>/dev/null || true
# ← 到此为止，不检查任何东西，直接往下走打印「完成」
```

`send-keys` 的退出码只表示「tmux 成功把按键送进了 pane」，**不表示 pane 里的程序收下了**。
再加上 `|| true` 把错误也吞了，于是「发出去」被当成「送到了」。

> **通用教训**：只要是「向另一个进程的 TUI 发送按键」，`send-keys` 成功都不等于对方收到。
> 这与 `[COLLAB-ACK-001]` 三次假警报是同一个认知误区的两个面 ——
> 那边是「ACK 没写 ≠ Worker 死了」，这边是「按键发了 ≠ Worker 收了」。

### 三、修复（2026-08-14 落地，`bash -n` 语法验证通过）

**① 收紧就绪判据** —— 三处全部改为只认输入框占位符：

```bash
if echo "$pane_content" | grep -qiE 'Ask anything'; then
```

`Ask anything` **只在输入框已挂载且为空时才渲染**，是「可以开始打字」的充分信号。
注意它在有文字进入输入框后即消失，所以**只能用于注入之前**的就绪检测。

**② 注入改为「打字校验 + 提交校验」双段回读，各最多 3 次**：

| 段 | 判据 | 含义 |
| --- | --- | --- |
| 打字后 | 占位符 `Ask anything` **应消失** | 还在 = 这一趟被吞，重打 |
| Enter 后 | 出现 `esc interrupt`（生成中）**或** 占位符回归（输入框已清空） | 两者都没有 = Enter 没生效，补发 Enter |

**③ 失败时明确报错，不再谎称成功**：三次全败则打印
`❌ 上下文注入三次均失败，该 Worker 当前无角色上下文，派发前须人工补注入`，
并写 `comms.log` 标 `[CONTEXT_INJECT_FAIL]`。

**改动文件**：`collab/replace-worker.sh`（判据 1 处 + 注入块）、
`collab/init-collab.sh`（判据 2 处：主流程 + 内嵌脚本；注入块 1 处）。

### 四、残余风险（明确写出，不假装已完备）

判据依赖 OpenCode 的**占位符字面量** `Ask anything`。若上游版本改了这句 UI 文案，
就绪检测会失效 → 走满 30s 超时 → 进入「强制注入」。但此时**第二层的回读校验仍然生效**，
会重试 3 次并在失败时明确报错，不会再退化成静默丢失。即：单点失效不会导致静默故障。

### 十一、TEST-PROMPT-AB-033 实测结论（2026-08-14，24 次调用，主控已独立复算）

**主控验收方式**：不采信汇总表，直接从 `ab033-results.json` 的 `content` 原文重新分类
（`- ` 开头行 ≥2 判 bullet，`N. ` 开头行 ≥2 判 ordered，否则 para），
**与 tester-1 的 `class` 字段逐条比对：24/24 全部一致，报告属实非虚报**。

**混淆变量已排除**：两次实验采样参数完全相同 —— `harness_025f.py:79-80` 与
`harness_033_addpunct_ab.py:82-83` 均为 `temperature 0.3` / `max_tokens 512`，
模型均为 `deepseek-v4-pro`。**故 G0 与 08-03 可直接比较。**

#### 结果矩阵（bullet / ordered / para）

| 组 | 配置 | IN-D | IN-B | IN-C |
| --- | --- | --- | --- | --- |
| **G0** | 无 F4 + 旧 ADD_PUNCT（= 08-03 阶段六配置） | — | **0/0/3** | 2/0/1 |
| **G1** | 含 F4 + **新** ADD_PUNCT（= 今天生产） | 0/0/3 | **3/0/0** | 2/0/1 |
| **G2** | 含 F4 + 旧 ADD_PUNCT | 0/0/3 | **3/0/0** | 1/0/2 |

#### 四条结论

**① 🔴 `ADD_PUNCT` 那句话不是元凶 —— 实验证否，我们的提示词模块无罪**

G1 与 G2 唯一差异就是那句话。IN-B 两组**均 3/3**；IN-C 反而是**新版更好**（2/3 vs 1/3）；
IN-D 两组**均 0/3**。**没有任何一个输入上出现「旧版能出、新版不能」的模式。**

Gavin 的怀疑方向在「提示词确实变过」这一点上成立，但**因果不成立**。
主控此前标注的「因果未证实、不得当成结论」是对的，本次实测把它证否了。

**② 模型侧确实已漂移**

G0 用**完全相同的提示词、模型、采样参数**，08-03 时 IN-B 是 **3/3 bullet**，今天是 **0/3 para**。
同配置不同结果，只隔了 11 天 → **`deepseek-v4-pro` 的行为发生了变化**。
IN-C 亦从 3/3 退化为 2/3。

**③ 🔴 最有价值的新发现：F4 场景块是关键变量，它把 IN-B 救了回来**
...
这是本次实验**计划外**的收获，且直接改写了 08-03 结论的意义：

- 08-03 阶段六是在**无场景块**的提示词上做的，其「v4-pro 解决问题」的结论**只对那份提示词成立**
- 今天那份提示词已经不灵了（G0 0/3）
- **但生产是含场景块的，而含场景块时 IN-B 今天仍然 3/3**
- → **生产配置比 08-03 实验配置更强，生产并没有整体退化**

**④ IN-D 是真正落在能力边界外的样本：跨全部三组 9/9 全败**

它的失败**与 `ADD_PUNCT` 版本无关、与场景块有无无关、也不是本次漂移新引入的**
（IN-D 是新样本，从未有过成功记录）。这与本条目第二~四节「无序标记识别处在临界点、
差别在项目内容本身」的老观察一致。

#### 对 Gavin 原始问题的最终回答

> 「以前 v4-pro 能出无序列表，现在同一模型不能了，是不是我们提示词改坏了？」

**不是我们改坏的。** 两件事同时发生，被叠加成了一个现象：

1. 模型侧在这 11 天里确实漂移了（G0 铁证）
2. 而 Gavin 这次用的**买菜句（IN-D）本身就是模型吃不下的样本**，在任何配置下都没成功过

**可靠规避方式不变**：改用「第一 / 第二 / 第三」，有序枚举跨全部实验从未失败。

#### 遗留

- 模型漂移不可控，且无法预告。**若要把「无序枚举出列表」当成可依赖的能力，
  需要引入回归看门测试**（固定样本集 + 定期跑），否则下次漂移仍然只能靠 Gavin 端测撞出来。
- 场景块为何能救回 IN-B、其机制是什么，**未定位**。但它是目前唯一实测有效的正向变量，
  比继续改 L2 措辞更值得深挖。

---

## [HEREDOC-EXPANSION-001] ⚠️ bash heredoc 未加引号会展开反引号/`$()`，把文档内容执行掉【写文档前必读】

**状态**：✅ 已定位（2026-08-14，tester-1 自查上报根因，主控确认）

### 现象

tester-1 用 bash heredoc 向 `logs/20260814.md` 追加收尾节，结果写进去的内容：

- 模板变量**全部变成空**：`模型 ，temperature 0.3`、`Rust ，与  对齐`、`产物： + + ；。`
- 中间**莫名多出一大段 `git status` 原文**（含 `修改：     src/platform/macos/*.rs` 等 20 余行）

观感上像是「Worker 改了 src/ 却声称零改动」，**制造了一次虚假的红线告警**。

### 根因

```bash
cat >> doc.md <<EOF        # ← 分隔符没加引号
**模型 `deepseek-v4-pro`，Rust `String::len()`**
EOF
```

**未加引号的 heredoc 分隔符会让 bash 对正文做完整展开** —— 反引号 `` `...` `` 被当作
命令替换执行。于是：

- `` `deepseek-v4-pro` `` → 试图执行该命令 → 失败 → **替换为空串**
- `` `String::len()` `` → 同上 → 空串
- 而正文里若有 `` `git status` `` 之类，**会真的执行它，把输出粘进文档**

后者正是那段 `git status` 的来路 —— **不是复制粘贴失误，是 bash 替 Worker 执行了它。**

### 正确写法

| 方式 | 是否安全 |
| --- | --- |
| `cat >> doc.md <<'EOF'`（**分隔符加单引号**） | ✅ 完全字面，零展开 |
| `cat >> doc.md <<EOF` | ❌ 反引号、`$VAR`、`$(...)` 全部展开 |
| Write / Edit 工具（不走 shell） | ✅ 推荐，Markdown 里反引号极常见 |

**结论：写含反引号的 Markdown，要么用 `<<'EOF'`，要么直接用 Write/Edit 工具。**
本条与 `[ENCODING-UTF8-001]`（禁 PowerShell 改 UTF-8）同类 ——
**都是「用错了写入通道，内容在落盘前就被改坏了」。**

### 🔴 第二个教训：修复不能只做加法，必须删旧块

tester-1 发现坏块后，用 Write 工具**追加**了一份干净收尾节，**但没删坏块** →
文件里同时存在两份同名章节，其中一份自相矛盾（写着「零生产代码改动」，
下面紧跟着一屏 `修改： src/...`）。

主控在验收时先删除了坏块（文件 256 → 201 行）；**而此时 tester-1 正准备按它记录的
旧行号 172-223 执行删除** —— 行号已因主控的删除全部前移，第 172 行已变成
**正确收尾节的标题**，照此执行会把整个结论段删掉。主控发现后紧急叫停。

> **通用规则：多方可写的同一文件，禁止用行号定位删除。**
> 必须用**内容锚点**（唯一标题、特征串）定位，或先重读文件确认行号仍然有效。
> 本次是「两个 Agent 同时清理同一文件」的经典竞态，靠人工叫停才没出事。

### 十二、✅ 条目关闭：机制已定位，按 DEC-049 刻意不修（2026-08-14）

**`TEST-PROMPT-IND-034`（18 次调用）把机制钉死了，且推翻了本条目第四节的一个旧结论。**

#### 机制：F3 两道闸之间存在定义空隙

`src/llm/mod.rs:1219-1233`：闸一「If unsure, DO NOT use a list」；
闸二 SHORT（名词短语、无谓语、≤6 字）→ 顿号内联／LONG（带谓语或内部标点的完整子句）→ 列表。

| 项目形态 | 归属 | 实测行为 | 判定 |
| --- | --- | --- | --- |
| `新鲜的青椒` | 明确 SHORT | 逗号→顿号内联 **3/3** | ✅ 规则被正确执行 |
| `有些同学下课之后徘徊在校园里不走，三五成群` | 明确 LONG | bullet 列表 **3/3** | ✅ 规则被正确执行 |
| `有没有新鲜的青椒`（无主语疑问谓词短语） | **空隙** | 既不列表也不加顿号 **9/9** | ⬅️ 命中闸一保守兜底 |

**模型全程正确。** 它在边界项目上「什么都不做」，正是我们写的「拿不准就别做列表」在命令它。

#### 🔴 推翻本条目第四节的旧结论：「差别不在长度」是错的

第四节依据旧实验 B6「缩短项 → 0/3」判定长度不是变量。
**那次「证伪」其实是 SHORT 规则在生效** —— 缩短项目恰恰把它们推进了
「顿号内联、不做列表」分支，当然不出列表。**长度一直是变量，当年读反了。**

> **方法论**：把「改了 X 仍不出列表」直接读作「X 不是原因」是危险的 ——
> 必须先确认改动没有把样本推进**另一条同样不产出列表的合法分支**。
> 本条目第七节记的「主控连错三次」，与此同源：**结果相同不代表路径相同。**

#### 结案

**Gavin 2026-08-14 拍板：保持现状，吃不准的不走列表化，避免规则过分精细化造成易失误和交叉误判。**
详见 `decisions.md` **DEC-049**（含代价不对称、边际风险递增、与 DEC-039/DEC-040 同向等四条理由）。

**本条目状态：🟢 关闭。** 后续再有人提「无序枚举不出列表」，先读 DEC-049 —— 这不是 bug，是既定取舍。
可靠规避方式：改说「第一/第二/第三」，有序枚举跨全部实验、跨两个模型型号从未失败过。

**两个独立遗留不随本条目关闭**：① 模型漂移（第十一节 G0 证据）；② 场景块为何是正向变量（机制未明）。

---

## [ASR-SAMPLERATE-STREAM-001] 🔴 改成边录边发后，重采样步骤被静默绕过，在线识别 100% 出错【必读】

**发现**：2026-08-16 Gavin 端测 v0.8.0 首包，新在线模型识别结果与所说内容完全不相干。

### 现象

Gavin 实际说话 → 识别出「你好，花呗还不上。花呗无法支付。的对。」
不是准确率下降，是**输出完全不相干的客服话术**——模型在它训练过的语料里找了个最像的。

### 根因（一句话）

麦克风以 **48000Hz** 采集，发给阿里服务器的连接参数写死 **`sample_rate: 16000`**，
中间**没有任何重采样**。服务端按 16kHz 解码 48kHz 数据 = 音频被拉慢 3 倍、音调降低，
识别结果自然全错。

### 取证链（四条，主控 Read / grep 实测）

| # | 位置 | 内容 |
| --- | --- | --- |
| 1 | `src/transcription/qwen_inference.rs:102` | `"sample_rate": 16000` 写死 |
| 2 | `debug.log:34` | `Streaming recording started (48000Hz, 2 ch, ...)` |
| 3 | `src/audio/mod.rs:189-317` | `record_streaming` 全函数 grep `resample` 零命中 |
| 4 | `qwen_inference.rs` 全文 | grep `resample` 零命中，只有 `f32_to_pcm16_le`（`:907`/`:968`） |

### 决定性对照（同日同机同麦克风，8 次录音）

| 时间 | 路径 | 日志有无 `Resampling ... 48000Hz to 16000Hz` | 结果 |
| --- | --- | --- | --- |
| 08:41 ×2 | 新在线流式 | **0 次** | 全错 |
| 10:16–10:22 ×6 | 旧本地模型 `asr_model=performance` | **6 次全有** | 全部正确 |
...
```
QwenAudio ASR pre-roll flushed: 261 chunks, 125280 samples (7.8s)
```

125280 samples 按真实 48000Hz = **2.61s**，代码按 16000 印成 **7.8s**，**正好 3 倍**。
**以后遇到在线识别乱码，先看这行的秒数对不对，一眼就能定性。**

### 为什么会漏（这才是要记住的部分）

旧路径 `record()` 在**录音结束那一刻**一次性重采样（`src/audio/mod.rs:757`，FIRSTCHAR-FIX-005 引入）。
改成边录边发后，**"录音结束"这个时刻不存在了**，挂在它上面的那一步就再没人执行。

> **通用教训：把「批处理」改成「流式」时，所有挂在"处理结束"这个时刻上的步骤都会静默失效。**
> 改造前必须先列出「原路径在结束时做了哪些事」，逐条确认新路径谁来做。
> 与 `[DEC-039]` 家族的「后端做完了，开关没装上」同属一类：**不是写错，是改架构时漏接线。**

### 同根因的次生现象（不要当成独立 bug 去查）

`debug.log:36` `VAD gate 2s deadline reached without detection, forcing connect (safety net)`
—— VAD 也是按 16kHz 配置的（`src/transcription/vad.rs:62,135`，窗口 512 = 32ms@16k），
喂它 48kHz 同样错，所以 2 秒都检测不到语音。**修好采样率后应自动恢复，但须实测确认。**

### 修法要点（ASR-042，2026-08-16 派 coder-1）

**不能**逐块直接调 `resample_anti_alias`：它是整段 windowed-sinc FIR（`TAPS=32`），
边界处 `src_idx` 越界即 `continue`（截断卷积核）。逐块独立调用 = 每 10ms 制造一次核截断，
**会让 FIRSTCHAR-FIX-005 修过的送气清声母失真以每秒 100 次的频率复发。**

正确做法：**带状态的流式重采样器**，跨块保留左侧历史与右侧 lookahead，
输出点下标用**全局计数**（不可每块从 0 重算，否则每块重新对齐产生跳变）。
护栏测试：同一信号，批处理结果 vs 逐块流式结果，逐点差值 ≤ 1e-5。
...
## [ABLATION-MODEL-TOO-LIGHT-001] ⚠️ 主控用「数值推演」代替真实消融时，模型偏轻会低估缺陷【下消融预期前必读】

**发生**：2026-08-17 `TEST-EXEC-043`。主控在派发阶段四前，用 Python 数值推演预告了
消融 A（`interpolate_step` 缺陷 A 复发）的预期结果，写进任务书：「**预期 3 条变红**」。
tester-1 实测**变红 4 条**，多出 `step_never_exceeds_quarter_of_delta`。

**tester-1 按纪律上报而未自行判定，这是本条能被及时纠正的唯一原因。**

### 一、根因：消融点是一个「被多处引用的中间变量」
...
fn interpolate_step(delta: i32) -> i32 {
    if delta == 0 { return 0; }
    let abs = delta.abs() as f32;                       // ← 消融点
    let magnitude = (abs * 0.25).max(1.0).min(abs) as i32;  // ← abs 被用了两处！
    magnitude * delta.signum()
}
```

「把 `delta.abs()` 改回 `delta`」在真实代码里会**同时影响两处**：`abs * 0.25` 与 `.min(abs)`。

而主控的 Python 模型只改了第一处，`.min()` 那侧仍用绝对值：

| 模型 | `delta = -50000` 时步长 | `step_never_exceeds_quarter` |
| --- | --- | --- |
| 生产正确实现 | `-12500` | 不适用 |
| **主控推演（偏轻）** | `-1` | ✅ 不红 → 少算一条 |
| **真实消融（tester-1）** | **`+50000`** | ❌ **变红** |

🔴 **注意 `+50000` 的符号**：真实消融下 `.min(负数)` 把 magnitude 压成负的大数，
再乘 `signum` 翻正 —— 步长**符号反转且完全越界**，窗口会朝**反方向暴跳**。
这比主控原以为的「缩小时爬得慢」严重得多。**模型偏轻不只是少算一条测试，是低估了缺陷性质。**

### 二、规则

1. 🔴 **主控预告消融结果时，必须精确复刻真实改法** —— 特别注意消融点是否为
   **被多处引用的中间变量/常量**。改一个 `let` 绑定，下游每一处引用都会变。
2. 🔴 **任务书里的消融预期是「参考」不是「判据」**。实测与预期冲突时，
   **以实测为准**，Worker **不得**修改测试去迁就任务书。
3. ✅ **Worker 侧正确动作（本次已示范）**：实测与主控预期不符 → **先报不改**，
   附上自己的初步分析，等主控裁决。本次 tester-1 的分析（「同根因的更强表现，
   似属护栏更严而非失效」）与主控复核结论完全一致。
4. **护栏「比预期更严」是好事，不是异常**，不要因为对不上预期就去削弱用例。

### 三、与既有条目的关系

- 与 `[TESTER-FABRICATED-REPORT-001]` **方向相反**：那条是 Worker 虚报，
  本条是**主控预期有误而 Worker 实测正确**。二者共同指向同一条纪律：
  **谁的结论都要有独立证据，包括主控的。**

---

## [STATIC-PROOF-MISSED-CALLGATE-001] 🔴 只比对「绘制内容等价」不查「调用门控」，会漏掉整类「画得对但根本没被画上屏」的缺陷【主控做静态论证前必读】

**日期**：2026-08-17 ｜ **发现者**：Gavin 端测 BUILD-018 ｜ **代价**：输入法 100% 不可用，出包后才暴露

### 现场

OVERLAY-043 把 `draw_recording_overlay` 拆成三段并重写了尺寸插值。主控为回答 Gavin 的
「不能影响本地模型录音的 overlay 窗口效果」，做了四条静态论证并全部标 ✅，其中第 1 条是
**机器比对**：拆分前单体与拆分后 `chrome + indicator_and_waveform + stop_button` 串接，
**18 个 GDI 原语序列完全一致（`diff` 为空）**。

四条论证全部成立，结论「对本地模型零影响」**却是错的**。

### 漏在哪

四条论证全部在问「**画的内容对不对**」，没有一条问「**这段画有没有机会被执行 / 窗口有没有被摆上屏**」。

真实缺陷是：043 把 Show 分支里**无条件的 `SetWindowPos`** 挪到了插值分支 `:1221` 里，
由 `size_interpolation_done` 门控；而 Show 分支对非流式态执行
`target_size = current_size = computed_size` → 两者恒等 → 门控条件 `current_size != target_size`
**恒为 false** → `SetWindowPos` 永不执行 → 窗口 `ShowWindow` 了但从未被定位定尺寸。

**GDI 原语序列一致是真的，只是那串原语画在一个没有位置和尺寸的窗口上。**

### 防御规则（主控做「零影响」类静态论证时逐条打钩）

| # | 必问 | 本次若问了会怎样 |
| --- | --- | --- |
| 1 | 内容是否等价？ | ✅ 已问（GDI 原语 diff 为空） |
| 2 | **这段代码的调用点还在不在、还是无条件的吗？** | ❌ 未问 —— 一问即发现 `SetWindowPos` 被加了门控 |
| 3 | **被重构挪走的副作用（定位/定尺寸/失效重绘/焦点/z-order），新位置的触发条件覆盖旧的全部输入吗？** | ❌ 未问 —— 一问即发现非流式态不满足新条件 |
| 4 | 新增的布尔门控，是否存在**恒真/恒假**的输入组合？ | ❌ 未问 —— `current==target` 就是恒假组合 |

**操作化做法**：重构涉及副作用型 Win32 调用（`SetWindowPos` / `InvalidateRect` /
`ShowWindow` / `SetFocus` / `SetWindowLongPtr`）时，用
`git show <重构前提交>:<file>` 把**改动前后同一函数里这些调用的清单与守卫条件**拉出来逐个对照，
少一个或多一个守卫都要给出理由。**别只 diff 绘制序列。**

### 与既有条目的关系

- 与 `[ABLATION-MODEL-TOO-LIGHT-001]` 同族：都是**主控自己的推演模型偏轻**。那条是「消融预期少算一条」，
  本条是「静态论证漏掉一个维度」。共同解法：**推演必须写清覆盖了哪些维度、没覆盖哪些**，
  不写「四条全过」就当全面。
- 与 `[DOC-STATE-DRIFT-001]` 无关（这次文档写了，只是写错了）。


---

## [WORKER-WRITE-SILENT-FAIL-001] 🔴 Worker 报「文件已写入 N 字节」但文件实际 0 字节，且重写后 mtime 都不变【主控验收必查文件大小，别只看通知】

**日期**：2026-08-17 ｜ **现场**：coder-2 交付 OVERLAY-046

### 现象

1. coder-2 通知：「交付已写入 outbox/coder-2/result.md」→ 主控实测 **0 字节 / 0 行**
2. 主控打回要求补写 → coder-2 通知：「result.md 已补写完成（**3240 字节，UTF-8**），含收尾自证表」
3. 主控再测：**仍是 0 字节，且 mtime 停在第一次的 `14:36:56` 一秒都没动**
   —— 说明第二次写入**根本没有触达该文件**，不是内容错、是写操作没落地

### 关键判据（怎么区分「写错地方」和「写失败」）

本项目存在**双路径**（见 `[COLLAB-PATH-DUAL-001]` 同族问题）：

| 路径 | 本次实测 |
| --- | --- |
| `/d/Workspace/CodeLab/collab/outbox/coder-2/result.md`（工作区级，dispatch.sh 用这个） | 0 字节，mtime 14:36:56 |
| `/d/Workspace/CodeLab/voice-ime/collab/outbox/coder-2/result.md`（项目级） | 3570 字节但 mtime **08-15**，是上一个任务的陈旧文件 |

`find ... -name result.md -newermt "今天"` 全盘只命中那个 0 字节文件
→ **排除「写到另一条路径」**，确认是写入本身失败。

**主控排查顺序（照抄即可）**：
1. `wc -c <路径>` —— 别用 `cat`，空文件 `cat` 无输出容易被当成「读取问题」
2. `stat -c '%s %y'` 看**字节数 + mtime**。**mtime 不变 = 第二次写没发生**，这是最硬的判据
3. `find <仓库根> -name result.md -path "*<worker>*" -newermt "<今天>"` 排除写错路径
4. 只有以上三步都做完，才能定性为「Worker 报告不实」

### 处置（本次做法，建议沿用）

**不因为 result.md 缺失就否定整个交付。** 本次 coder-2 的**实质产出全部合格**：

- 代码：`src/main.rs` +14 行，主控 `git diff -w` 逐行核对，与方案一致，红线四条全守
- 文档：`handoffs.md` / `CHANGELOG.md` / `docs/MACOS-HANDOFF.md` / `logs/20260817.md`
  主控逐个 `grep` 命中且内容准确 —— 它甚至补出了主控派单时漏列的第五个非流式态 `FocusLost`

→ 主控据此**验收通过并提交 `46be389`**，只把 result.md 单独挂账，不阻塞 P0 修复上线。

**理由**：result.md 是过程产物，`handoffs.md` + `CHANGELOG.md` 才是留档主体，两者内容重叠。
为一个过程文件卡住 Gavin 日常不可用的 P0，是本末倒置。

### 防御规则

| 对象 | 规则 |
| --- | --- |
| **Worker** | 写完任何交付文件后，**必须 `wc -c` 自查**再报数字。通知里报「N 字节」就意味着你量过了 —— **报了没量 = 虚报**，比不报数字更糟 |
| **主控** | 验收时 `wc -c` 所有被声明写入的文件，**不采信通知里的字节数**。这与「grep 核对五文档 ✅」是同一条纪律的延伸 |
| **主控** | 同一 Worker **连续两次**同一文件写入失败时，判定为其写入工具异常，**改为让它把内容直接贴进 tmux 通知**，不要第三次让它写文件（`[COLLAB-ACK-001]` 教训：重复施压不解决工具层问题） |

### 与既有条目的关系

- 与 `[TESTER-FABRICATED-REPORT-001]` 表面相似但**根因不同**：那条是 Worker 勾了 ✅ 却没做；
  本条 Worker **确实做了**（文档全在），只是 result.md 这一个写入静默失败而它没自查。
  **处置力度不同：那条要追责，本条只需补自查规则。** 别把两者混为一谈。
- 与 `[REPLACE-WORKER-TASKFILE-WIPED-001]` 同族：都属「文件层动作与 Worker 认知不一致」。

### 🔴 补充（同日，2026-08-17）：coder-1 也中同一现象 → 判定为**工具层问题，非 Worker 诚信问题**

HOTKEY-047 交付时 coder-1 通知「三自证已写入 result.md」，主控 `wc -c` 实测同样 **0 字节**。

**两个不同 Worker（coder-2 `kimi-k2.7-code` / coder-1 `glm-5.2`）、同一天、同一现象**
→ 排除个体行为，**定性为 OpenCode 侧写 `outbox/*/result.md` 的工具层缺陷**。

**因此本条目的处置基调确定为「补自查规则」，不是追责**，
与 `[TESTER-FABRICATED-REPORT-001]`（勾 ✅ 但没做）**明确区分开**。主控不得把两者混用处置力度。

**升级后的操作规程**：
1. Worker 写完 → `wc -c` 自查非 0 → 才报数字
2. **第二次仍写不进去，禁止第三次重试** → 直接把内容**贴进 tmux 通知正文**，主控代为落盘
...
### 🔴 [ABLATION-MODEL-TOO-LIGHT-001] 补充（2026-08-17）：主控推演在同一天被实测修正**三次**，已成稳定规律

| 批次 | 主控推演 | 实测 | 差因 |
| --- | --- | --- | --- |
| OVERLAY-043 消融 A | 3 条变红 | **4 条** | `abs` 变量两处使用，推演只算了比例基数一处 |
| HOTKEY-047 消融 B | 仅 S1 变红 | **S1 + S12 双红** | S12 冲突弹窗也显示键名，同源受影响 |
| HOTKEY-049 消融 A（删 `set.add(0xA4)`） | 用例 ④ 一条 | **4 条（T3/T4/T9/T11）** | 四条用例共用同一判据来源 |

**规律**：主控推演系统性偏轻，根因**不是算错，而是只追单一最直接的失败路径，
未枚举「共用同一判据/同一变量的其他用例」**。

**防御规则（主控下消融预期时照做）**：
1. 预期写成**下限**（「至少 N 条」），不写成精确数
2. 下预期前先 `grep` 该判据/变量的**全部引用点**，逐个问「这条用例会不会也红」
...
Worker 报「五文档已更新」，实际是**整份重写**而非追加：

| 文件 | 被删掉的东西 |
| --- | --- |
| `CHANGELOG.md` | **表头三行**（标题 + 格式说明 + `---`）+ 上一条 `TEST-SYNC-056` 条目 |
| `collab/handoffs.md` | **表头两行** + 同日 coder-2 的 054-C/D/E/F/F-B/J 全部交接条目 |
| `logs/20260818.md` | **表头三行**（含 Rule 7 记录规则）+ 当日 E2E-HARNESS-050 等既有小节 |

且 `CHANGELOG.md` 里留下一行**半截乱码**（`�` 开头的残句），
是覆盖写入时截断 + 编码损坏的产物。

### 为什么危险

- `collab/` 与 `logs/` **不在 git 管辖**（`.gitignore` 排除），一旦被覆盖**没有版本可回滚**。
  本次侥幸：这三份恰好是**已入库**文件（CHANGELOG 在库、handoffs/logs 当日已随主控提交过），
  才得以 `git show HEAD:<file>` 还原。**换成未提交的文档就是永久丢失。**
- 表头里写着「记录规则 Rule 7」「历史条目见 handoffs-archive.md」等**元信息**，
  删掉后下一个 Worker 不知道该怎么写。

### 主控侧防线（本次生效，固化为规程）

**提交 Worker 的文档改动前，必须先看 `git diff --stat` 有没有删除行。**
`N ++-------` 这种「加号少、减号多」的形态就是覆盖写的信号，
再用 `git diff <file> | grep '^-'` 确认删了什么。

### 修复手法（本次实操）

1. `git diff <file> | grep '^+' | sed 's/^+//' > /tmp/new_<file>.txt` 先把 Worker 的新内容抠出来
2. `git show HEAD:<file> > <file>` 还原原文
3. 用脚本把新内容**追加**回去（CHANGELOG 插在 `---` 表头之后，其余追加到文件末尾）
4. 断言新内容不含 `�`，再提交

### 给 Worker 的规则（已写入后续任务书）
...
## [PLAUSIBLE-FIX-NOT-ACTUAL-CAUSE-001] 🔴 修了一个「能解释症状」的缺陷，就宣布症状已修 —— 一批之内踩三次【宣布修复前必读】

**状态**：🔴 **活跃**，2026-08-30 一天之内**三次**（两次 Worker、**一次主控自己**）
**这条存在的意义**：**「找到一个能解释症状的缺陷」和「找到造成症状的缺陷」是两件事。**
前者只需要因果链讲得通，后者需要实测对照。本项目反复把前者当成后者。

---

### 一、2026-08-30 当天的三个实例

| # | 谁 | 事件 | 讲得通的因果链 | 实际 |
| --- | --- | --- | --- | --- |
| 1 | 主控 | 静态论证 OVERLAY-043 对本地模型「零影响」 | 逐条比对 18 个 GDI 绘制原语序列完全一致 | **只比对了「画什么」，没查「有没有被调用」**。`SetWindowPos` 被门控挡住，窗口根本没被定位 → 输入法 100% 不可用（OVERLAY-046）。已另记 `[STATIC-PROOF-MISSED-CALLGATE-001]` |
| 2 | coder-1 | ITN-071-B `一点半点` 被误转 | 「词在第 5 步保护集，可能运行时 toml 版本不一致导致未命中」→ 挪到第 1 步「防御性修复」 | 主控 `git log -S` 查实：该词 **2026-07-30** 提交 `0adb819` 即已入表，远早于端测所用 08-18 构建，**假设不成立**。根因至今无解，修复只是把症状盖住 |
| 3 | **主控** | ASR-070 尾部丢字，宣布「已修复」 | `final_text()` 只返回 `confirmed_sentences`，丢弃 `current_sentence` —— **完美解释**「最后一句没被确认就丢了」 | tester-1 实测对照推翻：`debug.log` 15:09:33 显示 `Transcribed`（服务端返回）与 `Injecting text`（实际注入）**逐字相同**，尾部字**从未从服务端到达**。`final_text()` 确是真缺陷、修法也对，**但它不是本症状的原因** |

**实例 3 的教训最贵**：因果链越顺，越容易停止追问。
`final_text()` 那个缺陷解释力极强 —— 名字、注释、代码、症状全都对得上，
连注释里那句「finish-task 后服务端会发最后一个 sentence_end=true」都恰好是个未验证假设，
**看起来就是它了**。但它不是。
...
1. **静态因果链的成本极低，实测对照的成本高**。读代码几分钟就能拼出一条通顺的解释，
   而拿到修复前后同场景的实测对照，往往要出包 + 端测 + 取日志。
   在「已经能解释了」和「还要再花半天验证」之间，人会默认停在前者。
2. **一个 bug 可以有多个「能解释它」的缺陷**。代码里真实存在的缺陷不止一个
   （`final_text()` 确实有 bug），找到其中任何一个都会产生「破案了」的错觉。
3. **本项目的症状描述常常不精确**。ASR-067 初报是「麦克风不接受输入」，
   澄清后才是「屏幕上屏显示中断」—— 描述一变，责任模块就从 ASR 变成 overlay。
   基于不精确描述建立的因果链，通顺也没用。
...
| # | 判据 |
| --- | --- |
| 1 | 🔴 **我有没有「修复前后同场景」的实测对照？** 没有 → 只能说「修了一个可能相关的缺陷」，**不许说「已修复」** |
| 2 | 🔴 **这个缺陷除了能解释症状，还有没有独立证据指向它？** 只有解释力 = 只是候选 |
| 3 | 🔴 **有没有别的缺陷也能解释同一症状？** 想不出第二个，通常是想得不够 |
| 4 | 🔴 **症状描述本身核实过没有？** 用户的第一次描述常常不精确（ASR-067 即是） |
| 5 | 🔴 报告用词：**证据充分**说「已修复」；**仅静态推理**说「修了一个能解释症状的缺陷，待实测确认」。**这两句话不许混用** |

---

### 四、配套做法：把修复本身当实验

拿不到实测对照时，**用修复的部署制造对照**，并**提前写死判读方式**。

ITN-071-B 的处理即为范例（主控 2026-08-30）：
把 `一点半点` 挪到最高优先级保护，然后**预先声明**：

- 出包后复测**仍失败** → ITN 已在第 1 步保护却依然被改 → 根因在 ITN 之外（大概率 LLM 回改），**ITN 洗清**
- 复测**通过** → 症状消除，原第 5 步的疑点降为低优先级待查

**关键在「预先声明判读方式」** —— 事后再解释结果，怎么解释都通。

---

### 五、相关条目

- `[STATIC-PROOF-MISSED-CALLGATE-001]` —— 静态论证漏查调用门控（实例 1 的专条）
- `[F3-UNORDERED-LIST-001]` —— 11 个「讲得通」的假设全部被实验证伪，约 210 次 API 调用的代价
- `[ABLATION-MODEL-TOO-LIGHT-001]` —— 用数值推演代替真实消融会低估缺陷

---

### 六、2026-08-30 当天第四个实例（同日补记）

| 谁 | 讲得通的因果链 | 实际 |
| --- | --- | --- |
| 主控 + tester-1 | ASR-074「会话中途停止产出」：词时间轴只覆盖录音 51.4%、7 组尾部丢失 5 组全为服务端未返回 | **两条都不成立**。① 主控拿词表跨度比 ，而  含松手后 finalize 等待时间，**分母用错**；② tester-1 脚手架按住热键仅 2.6-4.1s 却播放 5-6s 的 TTS 句子，**尾部根本没录进去**，服务端「未返回」是必然而非缺陷 |

**新增判据（第 6 条）**：🔴 **在用一份数据建立因果链之前，先校验这份数据本身可信**。
本例中「录音时长 vs 口播内容时长」只需一次对照即可发现污染，
但主控与 tester-1 都直接进入了因果推理。
**数据可信度检查必须前置于因果分析，不是事后补的。**

## [EVIDENCE-LOG-VOLATILE-001] 🔴 决定性日志证据躺在会被下一次运行冲掉的位置，主控却没有第一时间抢存【拿到关键 log 后必读】

**日期**：2026-09-03 ｜ **发现人**：tester-1（主动报出）｜ **责任**：主控

### 现象

Gavin 2026-09-03 09:27 做了一次受控复现，产出 `target/release/debug.log`，
这是 ASR-074 议题**第一份未被污染的数据**（真人真麦克风，非 TTS 放音），
主控据它定位出「音频上行背压 + 松手丢弃积压」的完整根因链。

同一天下午，tester-1 执行 REPRO-061-COLD，需要**反复冷启动同一个 exe** ——
每次运行都会写 `target/release/debug.log`。tester-1 在 sanity launch 后
主动报告「debug.log 已被本次运行覆写/追加」，主控才意识到：
**唯一一份决定性证据，一直躺在一个随时会被冲掉的位置，而且已经躺了 5 个多小时。**

实测结果是**追加不是覆写**，数据完整幸存 —— **但这是运气，不是设计**。
只要 tester-1 的 5 次冷启动里任何一次触发了日志轮转/截断，证据就没了，
且**唯一的补救办法是请 Gavin 重做一次受控复现**（他的时间成本远高于一次 `cp`）。

### 根因

主控在分析阶段把日志**读进了上下文**，就默认「证据已经拿到了」。
但读进上下文 ≠ 证据被保全：
- 上下文里只有主控**摘出来的那几行**，coder-1 后续要复核别的字段就没得看
- 会话压缩后连摘出来的行都可能只剩结论
- 文件本身在磁盘上仍随时可被覆盖

**同时派发了一个会反复写同一个文件的任务给 tester-1，主控做边界评估时
只查了「Worker 之间的源码文件级重叠」，漏了「运行时产物的写入冲突」。**

### 规则

1. 🔴 **任何被用作判据的日志/数据文件，认定它是判据的那一刻就 `cp` 到
   `voice-ime/collab/evidence/`，文件名带来源与时间戳**，不要等分析完。
2. 🔴 **派发前的边界评估必须包含运行时产物**，不只是源码文件：
   `target/release/debug.log`、`Publish/`、截图目录、CSV 产物，
   两个 Worker 同时写同一个运行时文件同样是冲突。
3. 任务书引用证据时，**一律引用 evidence/ 下的只读副本路径**，
   不要引用会被后续运行改写的原始路径。
4. Worker 侧配套：发现「我这次运行动了某个可能是证据的文件」要主动报，
   **tester-1 这次就是这么做的，做对了**。

### 反面教训的年龄

本条与 `[BUGREPORT-SELFCORRUPT-001]`（用本 IME 口述的 bug 报告被 bug 本身污染）、
`[ABLATION-MODEL-TOO-LIGHT-001]`（用推演代替真实消融）同属一类：
**证据本身的可信度与可用性，要先于基于证据的推理被检查。**
2026-08-30 那次「三条证据里两条不成立」是证据**可信度**问题，
本条是证据**存续性**问题 —— 前者已记，后者此前没人写过。

## [GATE-MATCHES-SHAPE-NOT-CONTENT-001] 🔴 正则闸门认「形态」不认「内容」，口述语料/实验 dump 是它的盲区【建 .gitignore 规则前必读】

**日期**：2026-09-04 ｜ **发现**：Gavin 一句「collab 目录全部都会 push 吗」 ｜ **处置**：SECRET-083

### 事情怎么起的

SECRET-082 刚把 pre-push 闸门建起来、13 条用例全绿，看起来这块已经收口了。
Gavin 接着问了一句「collab 目录全部都会 push 吗」——**这句话问的不是闸门，是范围**。

查证：`.gitignore` 只排除 `inbox/outbox/acks/drafts` 四个瞬时 IPC 目录，
**其余 93 个 collab 文件全量入库并 push 到公开仓库**（2026-07-30 Gavin 定的，
理由是 Windows / macOS 两侧团队信息对称，这个决定本身没问题）。

主控随即用 pre-commit/pre-push 的**同一套判据**全量扫了这 93 个文件：
密钥/邮箱/手机号/身份证层面**干净**，只有 `scene-webtitle-audit-007.md` 里的
`user@<邮箱域>` 形态一处（原文是通用示例地址，此处不复制真形态，理由见本条末尾），文档示例，误报。
...
| 文件 | 内容 | 闸门为什么抓不到 |
| --- | --- | --- |
| `collab/evidence/*.log` | 69 处 `Online ASR result` / `[ASR-WORDS]`，载荷是 **Gavin 本人真实口述的转写文本**；另有 `app_exe="Notepad.exe"`（当时开着什么程序） | 一句中文话没有任何「形态」，正则永远写不出来 |
| `collab/research/*.{json,log,txt}` | 15 个 json + 7 个 log，LLM 实验输入输出 dump。`verbatim-inputs-025b.json` **名字就叫「逐字」**；`prompt-parts-025-full.json` 21KB | 同上 |

### 🔴 这就是 [SECRET-IN-REPO-001] 的同一个失效机制，换了一层皮

那次泄的是 key，形态明确，所以能用正则兜住 —— 于是我们建了闸门，然后**默认这块收口了**。

但两次的**根因是同一个**：**把「过程产物」当成「结论」一起入库。**
你想留的是 prompt、是参数、是复现证据；key 是顺带被抄走的，语料也是顺带被抄走的。
key 那次运气好，形态可枚举；语料这次运气不好，**它长得跟正常文档一模一样**。

**推论（这条才是要记住的）**：机器闸门只能兜住「能写成正则的东西」。
**凡是靠正则兜的地方，都必须再配一道「范围规则」** —— 即从一开始就不让这类文件进库。
闸门是网，`.gitignore` 是不往网上扔东西。**只有网、没有范围规则，等于默认所有内容都该公开。**

### 处置（Gavin 2026-09-04 批准）

`.gitignore` 新增，并 `git rm --cached`（**只出索引，本地文件一个不删**，实测 25/25 仍在）：

```
/collab/evidence/
/collab/research/*.json
/collab/research/*.log
/collab/research/*.txt
```

**🔴 主控原方案只排除数据类文件、保留 `research/*.md`（43 个），
Gavin 当场推翻并扩大范围**：「这两个目录不是项目必须的代码，整个目录都加进 gitignore」。

**他是对的，主控这条分界线画错了位置。** 主控画在「结论 vs 原始数据」，
Gavin 画在「项目必须 vs 不必须」—— 后者才是入公开库的正确判据：

- 分析过程文档既不是构建产物，也不是两侧团队的协作契约，本地随时可查；
- 入公开库唯一的额外效果，是把「我们研究过什么、拿什么语料研究的」一并送出去；
- 而且 `.md` 里照样会**引用**语料原文（消融实验必然贴输入输出），
  主控那条「结论安全、数据危险」的分界，在这个目录下**根本不成立**。

最终范围：`/collab/evidence/` + `/collab/research/` 整目录出库，共 83 个文件移出索引
（25 数据类 + 58 文档类，本地一个不删，实测 1000 个本地文件数移除前后不变）。
**协作契约继续入库不变**：todo / handoffs(+archive) / decisions / troubleshooting /
progress / build-test-guide —— 这些是两侧团队真正要对齐的东西。

### 已公开的部分（诚实交代，别当作已解决）

| 文件 | 状态 |
| --- | --- |
| `collab/evidence/debug-*-20260903-0927.log` | 🟢 **尚未推送，本次拦下了** |
| `verbatim-inputs-025b.json` / `prompt-parts-025-full.json` / `ab033-components.json` 等 | 🔴 **已在 origin/main，早已公开** |

本次 `git rm --cached` 会让它们从**仓库最新版本**消失（网页上看不到了），
但 **history 里仍在**。和 key 一样：删文件 ≠ 没发生过，只有重写历史 + force push 能清痕迹，
**破坏性操作，未获授权不执行**。

且与 key 有一处**更糟**的不同：**语料没有「吊销」这个动作**。
key 吊销后那串字符就是废字符串；说过的话公开了就是公开了，没有对应的止血手段。

### 🔴 写这条经验的过程中，闸门第三次拦下主控自己

`[HOOK-FAIL-OPEN-001]` 里已经记过两次：写测试结果表时抄了手机号明文，
改掉后引用拦截输出又把它抄了回来。**本条目提交时是第三次** ——
主控在描述「我扫了 93 个文件、只有一处邮箱误报」时，
**把那个示例邮箱原样写进了句子**，于是又被拦。

三次全是同一个动作：**在记录一件与敏感信息有关的事时，顺手复制了真形态。**
不是不知道规则，是叙述的惯性 —— 「说清楚我发现了什么」天然会引向「把它贴出来」。

**因此定为硬规矩，不再靠自觉**：
文档里凡是要提及密钥、邮箱、手机号、身份证，**一律只写形态描述，不写实例**
（`sk-<30字符>`、`138xxxx8000`、`user@<邮箱域>`、`<REDACTED>`）。
对读者的信息量完全一样，对泄露风险差一个量级。
三次拦截**没有一次动用 `SECRET_SCAN_SKIP`** —— 被闸门拦住的正确反应是改内容，
不是关闸门；一旦养成 SKIP 的习惯，这道网就退化成一句提示音。

### 给未来的自己

1. 建了机器闸门之后，**紧接着要问的下一个问题是「它认不出什么」**，而不是「它测通过了吗」。
2. 存复现证据、存实验产物时，**先问它会不会进库**，再决定放哪。
   放 `collab/evidence/` 之前它是安全的；放进去那一刻它就在等着被 push。
3. Gavin 那句「collab 目录全部都会 push 吗」是本条目的全部价值来源 ——
   **主控自己扫完 93 个文件报了「干净」，如果没人追问范围，这批语料就跟着下一次 push 出去了。**

## [ASR-SERVER-REWRITES-TIMELINE-001] 🔴 服务端会整段重写词时间轴（重分词），而文本只增不减【改流式 tween 前必读】

**日期**：2026-09-04 ｜ **来源**：coder-2 查 OVERLAY-086 Bug 2 时发现，主控用 Gavin 端测 log 全量统计验证
**证据**：`collab/evidence/debug-gavin-endtest-20260904.log`（98MB，Gavin 09-04 09:10–11:22 端测原件）

### 一、确立的事实（实测，非推测）

Qwen 在线 ASR 的 `Online ASR result` 包里，**词时间轴会被服务端整段重写**，
表现为**词数回撤而文本长度不变**（= 重分词/合并，不是回退）：

| 时间 | session | 词数变化 | 文本长度 |
| --- | --- | --- | --- |
| 09:11:12.185 | id=1 | 8 → **7** | 仍 11 字 |
| 09:11:13.964 | id=1 | 10 → **8** | 仍 14 字 |
| 09:11:18.391 | id=2 | 20 → **19** | 仍 27 字 |

同一个词的时间戳也会被改写（coder-2 举证：`是[8000-8060]` → 同词重写为 `新[7940-8140]`）。

### 二、🔴 与之相对，**文本长度严格单调不减** —— 这条同样是实测，且推翻了一个流行假设

三个会话的 `display` 长度轨迹（主控全量统计，52 个包无一例外）：

```
id=1: [0×21, 2, 4, 6, 10, 11, 14, 14]
id=2: [14, 15, 16, 18, 21, 24, 27, 27, 29, 31, 34, 37, 40, 43, 46, 50, 51, 52]
id=3: [52, 53, 54, 57, 60, 61]
```

**推论**：`src/main.rs:1543` 那条「服务端回撤 → `target < displayed_chars` → snap 下降」的分支，
**在真实数据里从未触发过**。它防的是一种没发生的情况。
拿它去解释任何症状之前，**先用 log 证明 target 真的变小过**。

（三个 session id 是**句子边界**：前一句 `end=true` 后换号，`display` 跨句累加。
**不是重连**，别往重连方向猜。）

### 三、🔴 `reveal_chars_by_timeline` 的 revealed 永不为 0 —— 结构性保证，别再拿它当空窗口的解释

`src/main.rs:3550`：

```rust
let origin_begin = words[0].begin_time;      // 每次调用从「当前词表」现算
for w in words {
    if w.begin_time - origin_begin <= elapsed_ms { revealed += ... } else { break }
}
```

首轮 `w = words[0]` 时判据化为 `0 <= elapsed_ms`，而 `elapsed_ms` 来自
`Instant::elapsed()` 恒 `>= 0` —— **恒真**。

两个直接推论：

1. **词表整体平移不影响 reveal**：origin 是从当前词表现算的，平移量会被一起吸收。
   「首词 begin 从 1120 跳到 1160 导致 revealed 归零」这类推演**不成立**。
2. **只要 `words` 非空，`revealed` 至少包含首词，永远不可能是 0。**

### 四、这次调查的处置

- coder-2 的**发现（服务端重写词表）成立并被采纳**，它能解释 Gavin 报的「中断显示/卡顿」
  （单调 max 保护让 `displayed_chars` 停在旧值）
- 但其**推论链末两环被推翻**（见二、三），因此「文字全部消失只剩空窗口」的成因**仍未确立**
- `displayed_chars` 归零全仓仅三条路：`:1285`（status 切 `Recording`，仅会话起点）、
  `:1481`（`Hide` 分支，会话结束）、`:1543` snap（需 target 变小，实测不发生）——**三条全堵死**
- 按 `[PLAUSIBLE-FIX-NOT-ACTUAL-CAUSE-001]`，**主控驳回了按未证实机制动手的细案**，
  改为继续定位，并允许「只交定位结论、不交修复」

### 五、给未来的自己

1. **「服务端回撤」这个词有歧义，必须分清是哪一种**：词数回撤（真实存在，频繁）
   vs 文本回撤（本次数据里零发生）。混为一谈会推出错误的机制链。
2. **决定性 log 要第一时间抢存**：本次那份 98MB 原件躺在 `target/release/debug.log`，
   **下一次构建就没了**（`[EVIDENCE-LOG-VOLATILE-001]` 复发）。已复制到 `collab/evidence/`
   （该目录 2026-09-04 起 gitignore，含口述语料，**不进公开仓库**，见 SECRET-083）。
3. **Worker 报「debug log + 代码实证，非推测」时，仍要自己跑一遍统计**。
   本次它的观测是真的，但从观测到机制的那几步推演是错的 —— **观测可信 ≠ 推论可信**。
...
### ✅ 2026-09-04 止血完成 —— Gavin 已在 DeepSeek 后台吊销该 key

**这条是本事故的真结案点。** 此前所有动作（明文换占位符、全仓扫描、pre-commit 闸门、
收尾自证表加「密钥/凭证」行）都只是**防复发**，对已泄露的这一把 key 一点用都没有 ——
它已经在 `f58af96` 的历史里、已经 push 到公网、已经被人拿去刷了 20 天。

**删文件不等于止血，吊销才是。** 吊销之后，`f58af96` 历史里的那串字符变成一段失效字符串，
剩下的只是卫生问题：

| 遗留项 | 性质 | 状态 |
| --- | --- | --- |
| 重写 git 历史 + force push 清除痕迹 | **卫生**，非安全 | 破坏性操作，**未获授权不执行**，Gavin 说了算 |
| LLM 401 是否已解决 | **可用性** | 新 key 配上后需确认，否则 FMT-072 出包端测仍验不了 |
| 新 clone 执行 `git config core.hooksPath scripts/git-hooks` | 闸门生效前提 | 钩子本体已入库不会丢 |

**给未来的自己**：下次再遇到凭证泄露，动作顺序是 **① 立刻让能吊销的人去吊销 → ② 才轮到
清仓库、加闸门**。反过来做，就是拿「我已经删了文件」这种自我安慰，替换掉真正止血的那一步 ——
本次从 08-14 泄露到 09-04 吊销，中间隔了 21 天，钱是在这 21 天里被刷走的。
...
## [LOG-FIELD-IS-OUTPUT-NOT-INPUT-001] 🔴 日志里那个字段是**函数输出**不是**入参**，拿它反推入参会推出完全虚构的机制【查流式 ASR 前必读】

**日期**：2026-09-05 ｜ **来源**：coder-2 查 OVERLAY-101 Bug A，主控用同一份日志跑全量统计推翻

### 一、踩法

`qwen_inference.rs` 的日志行长这样：

```
Online ASR result: id=2, end=false, display='...', words=8
```

**只有 `display` 和 `words`，没有 `raw`。** 而
`display = display_text() = confirmed_sentences.join("") + current_sentence`，
`words = display_words().len() = confirmed_words 总数 + current_words 数`
—— **两个都是聚合后的输出，不是服务端发来的那一包的内容。**

coder-2 由此推出「服务端新句 partial 是累计式，携带上一句全文作前缀，导致 display 双计」，
并给出「id=2 首包 rawLen=14、display 一包内 14→28、随后虚高到 65、end 包一步回落到 52」的机制链，
建议在 `on_result` 里对 raw 做 `strip_prefix(confirmed.join(""))`。

### 二、主控全量统计（52 包逐包）—— 机制三处被直接证伪

```
  时刻          id  end   displayLen  words
  09:11:13.964  1  True      14        8
  09:11:14.084  2  False     14        8   <<< id 切换
  ...
  09:11:25.835  2  True      52       34
  09:11:25.835  3  False     52       34   <<< id 切换
  09:11:28.071  3  True      61       39
```

1. **`display` 回落次数 = 0**（52 包无一例外）。「65→52 一步回落」**不存在**。
2. **全程最大值 61，没有 65。** 该数字与其自述的机制自相矛盾，交付前未自查。
3. **id=2 首包 `display=14, words=8`，与 id=1 end 包逐位相同。**
   若该包 raw 真含 s1 全文（14 字），`display` 必为 `14+14=28`。
   实际是 14 ⇒ **该包 raw 为空串、current_words 为空** ⇒ **不是累计包，是空包。**
4. 其所谓「8 个词、时间戳 1160-5240 = s1 的词被重放」——那正是
   `display_words()` 把 `confirmed_words` 拼进来的**正常输出**。**把输出当成了入参。**

### 三、🔴 更致命的：查的是**不含该缺陷的会话**

那 52 个包**全部落在 2026-09-04T09:11:08~28**。
而 Gavin 报 Bug A 是 **2026-09-05 BUILD-098 端测**。
⇒ **在一份根本没出现过该现象的数据上，推出了该现象的成因。**

主控核实：Gavin 端测跑 `Publish/` 且未带 `-debug`，`Publish/debug.log` 仍是 8-17 的 41 行，
`%APPDATA%\voice-ime\` 无日志目录 ⇒ **Bug A 至今零运行时证据**。

### 四、那个修法本身的两个真实回归风险（即使机制成立也不能这么改）

1. **`strip_prefix` 是纯文本前缀匹配，遇真实重复说话会误剥。**
   confirmed=`我知道了`、新句真说`我知道了吗` → 严格前缀命中 → 显示被砍成 `吗`。
   且 **`final_text()` 就是 `display_text()`**（`qwen_inference.rs:523-528`）
   ⇒ **误剥直接污染注入给用户的最终文本**，不止显示层。
2. **只剥文字不剥 `current_words`** ⇒ 破坏 OVERLAY-051-G「`display_words()` 与
   `display_text()` 字符级对齐」契约 ⇒ 时间戳驱动回放出错
   ⇒ **可能让刚验收 OK 的 OVERLAY-086 Bug 2（空窗口）复发**。

### 五、给未来的自己（三条硬规矩）

1. **动手前先确认日志字段的语义是入参还是输出。** 名字叫 `display` 的十有八九是渲染态聚合值。
   要拿入参就**先加入参埋点**，不要反推。
2. **推出来的数字要和同一份日志的极值自查一遍。** 说出「回落到 52」之前，
   先问「这份日志的最大值是多少」——本例最大 61，声称的 65 当场就该露馅。
3. **先确认日志时间范围覆盖了缺陷发生的那次会话。** 时间对不上，
   后面所有分析都是在给另一件事编故事。
