...
**主控建议（已修正）**：复议 DEC-015，改为 **objc2 + NSApplication + CFRunLoop 直驱**，结论写成 DEC-036。
...
### ✅ 已由 Gavin 拍板（2026-08-04）→ **DEC-036，主控的降级建议被推翻**
...
**主控已提取的 Windows 侧规格基线**（macOS 实现的对齐依据，全部实测自 `src/main.rs`，完整表见 `decisions.md` DEC-036）：
