...
## DEC-062 · Q3 根因修复：预窄模型切换的「首次录音用旧引擎」窗口（FIX-164，部分修订 DEC-025，Gavin 2026-09-07 拍板）

### 背景

DIAG-163 实锤（代码级五步证据链）：DEC-025 的「重建异步进行，期间继续用旧实例不阻塞录音」
存在一个从未被点破的后果——**模型切换后的第一次录音必然用旧 transcriber**（懒触发：needs_reload
只在 Start 处理器内判定，用户不按热键就永远不重载；异步重建也不等）。控制器按 config 先画
流式占位窗，worker 按「config **且** 实例」双条件走了本地管线，无条件 RecordingStarted 把
占位窗覆盖成本地频谱窗。

**定性修正（Gavin 拍板，推翻 DIAG-163 初稿「只错窗口外观」）**：Gavin 选了在线 ASR，那一次
录音的转写**实际由旧的本地模型产出**——用户拿到旧引擎的识别质量却以为在用新引擎。这是
**静默的功能性错误**，不是外观问题。因此「接受现状 + 文档标注」出局。

### 决策（方案 D = D1 预热 + D2 有界等待；否决单纯同步等待的建议 A）

| 项 | 决策 |
| --- | --- |
| D1 预热 | worker 空闲 tick（recv_timeout 500ms）轮询**廉价层**判定（模型身份 + 在线 key/url/model，**不读词库**），配置变更 ≤500ms 内后台启动重载。实现方式取 worker 轮询而非任务书初拟的「配置变更通知」：写 config 的唯一路径是文件监视线程，通知需跨线程管线改动（watcher 签名 + 两处调用点），轮询零管线改动且时延被 D2 完全吸收（协商 2 轮，主控采纳） |
| 词库取舍 | 词库版本变更**不预热**（判定要读 SQLite），只在 Start 完整判定触发——Q3 病根是引擎身份，词库走原路径够了（主控拍板） |
| D2 兜底 | Start 决策管线前，若廉价层命中且重载在途，最多等 **1500ms**；逐 ≤100ms 切片轮询，可被 stop/cancel 信号随时打断；**超时/失败 ⇒ 退回旧行为（旧实例继续），绝不挂死热键路径（硬红线）** |
| 防风暴 | 重载失败记录失败签名 (期望模型, key, url, model)；空闲预热对同一签名不自动重试；配置再变签名即变恢复预热；Start 主动路径不受限 |
| 不动项 | `RecordingStarted` 的发送时机/位置**零改动**（HOTKEY-LATENCY-FIX-001 地盘）；判定函数 `asr_cheap_reload_needed` / spawn `spawn_asr_reload` / 结果应用 `apply_reload_result` 三处收口，预热与 Start 共用，不许两份漂移 |
| macOS | `spawn_worker_thread` 已平台中立（MACOS-P4-NEUTRAL-002），D1/D2 两端同效；见 docs/MACOS-HANDOFF.md |

### 与既有决策的关系

- **部分修订 DEC-025**：「重建异步不等」保留（idle 期），但 Start 决策前有了 1.5s 有界等待 +
  空闲预热两道闸，「首次录音用旧引擎」从**必然**降为「重载 >1.5s 且用户秒按热键」的残余窗口。
- 不推翻 ASR-056 双条件判据本身；D2 把「判据取样时刻」从 spawn 后立即改为等待落地后。

### 影响

- 文件：`src/main.rs`（worker 重载区 + mic_pulse + 菜单图标日志）；平台中立，两端同效。
- 行为变化：切模型后空闲 ≤500ms 即开始后台重建（多一次模型加载的内存/CPU 峰值，与原
  Start 内重载同量级）；Start 在重载在途时最多多等 1.5s（仅廉价层命中时）。
- 风险控制：等待必有超时；失败签名防风暴；录音期间（loop 顶不执行）零重载行为。

### 遗留

- 残余窗口（重载 >1.5s + 用户秒按）仍会退回旧引擎——如端测仍见错窗，再加长上限或把
  D2 等待改为「等待期间先不发 RecordingStarted」的方案（需重新评估热键延迟回归）。
- Part B 声波弧（阈值 0.35→0.10 + 0.35 地板）与 Part C 菜单图标日志随本 DEC 一并交付，
  验收以 Gavin 端测目视为准；Part C 是永久日志不是探针，不进 PROBE 删除清单。

---

## DEC-063 · LLM `temperature` 维持 0.3 不动：温度不是格式化稳定性的杠杆（Gavin 2026-09-10 拍板）

### 背景

Gavin 提问：「temperature=0.3 针对我们的场景，是不是要再调整一下」。
起因是 VERIFY-206 期间观察到列表化时出时不出（当时记为 9/10），怀疑随机性所致。

### 实测（真实 API，Gavin 授权用生产 key，30 次调用）

模型 `deepseek-ai/DeepSeek-V4-Flash`。三档 × 两用例 × 各 5 轮：

| temperature | 用例A（显式「比如/再比如」并列三项） | 输出种数 | 用例B（无标记，纯语义判断） | 输出种数 |
| --- | --- | --- | --- | --- |
| 0.0 | **5/5 出列表** | 2 | 0/5（保持段落） | 2 |
| 0.1 | **5/5 出列表** | **1** | 0/5（保持段落） | **1** |
| 0.3（现状） | **5/5 出列表** | 1 | 0/5（保持段落） | 2 |

**两个用例的判断在三档下完全一致**（A 全出列表、B 全不出），差异仅在措辞的细微抖动。
且 B 组不出列表是**正确行为** —— F3 明写 `Over-formatting ordinary narration into lists is a regression`。

### 决策

**维持 `temperature: Some(0.3)` 不动**（`src/llm/mod.rs:710/798/922` 三处硬编码）。

### 原因

1. **温度不是杠杆，提示词才是。** 此前观察到的 9/10 不稳定，根因是老 config.toml 的陈旧基座
   （VERIFY-206：旧基座 2/10、新基座 9/10）。基座修复后同一用例三档均 5/5。
2. **三档核心判断无差异**，收益仅剩「措辞少抖一点」，属边际。
3. **改动影响面 3 倍于收益**：三处分别服务纯翻译 / 优化+翻译 / 主路径优化，
   改它等于同时改纠错、格式化、翻译三个维度，而本次只验证了格式化一个维度，
   需重跑三维度验证 + 重新出包。按「验证成本与风险成正比」不划算。

### 🔴 顺带纠正一个常见误解：`temperature=0` 不等于确定性

t=0 在**两个用例上都出现 2 种输出**，稳定性反而不如 0.1 / 0.3。
API 端的浮点非确定性与批处理调度决定了 0 不保证逐字复现。
**不要把「要确定性就设 0」当成定论。**

### 若将来重启此议题

- `0.3 → 0.1` 是唯一有实测支撑的候选（唯一在两用例都做到 1 种输出的档位），方向也契合
  「确定性纠错任务」的场景性质，且不是激进的 0。
- 改则三处一并改，并**必须补测纠错与翻译两个维度**，不能只看格式化。
- 🔴 **不建议把它做成用户可配项** —— 违反 DEC-031 单开关原则与「默认零配置」方向。

---

## DEC-064 · 长文档改「索引 + 全文归档」两层结构（Gavin 2026-09-10 拍板）

**背景**：主控实测启动必读文档合计 **≈227,865 token**，而 CodeLab `CLAUDE.md` 规定的上限是
**~20,000 token —— 超标 11 倍**。构成：`troubleshooting.md` 94.7K（110 条）+ `todo.md` 69.3K（56 节）
+ `decisions.md` 50.0K（67 条）+ worker-guide 5.9K + 三份 CLAUDE.md 6.0K。

其中绝大部分是**历史记录**，不是启动时需要知道的东西。但内容本身不能删 ——
它们是项目吃过亏换来的判断（`[SECRET-IN-REPO-001]` 的真金白银、`[GIT-RESET-INCIDENT-001]` 的 11 个批次）。

**Gavin 拍板**：走「索引 + 归档」。

| 启动读的（索引） | 全文去处 | 粒度 |
| --- | --- | --- |
| `todo.md` | `todo-archive.md` | 一条待办 ≤8 行，只留「是什么 + 前置/风险 + 指针」 |
| `decisions.md` | `decisions-archive.md` | 一条 DEC 一行「判据 + 状态」 |
| `troubleshooting.md` | `troubleshooting-archive.md` | 一条坑一行「现象 → 判据」 |

**为什么是这个粒度**：启动时需要的是**「知道有这么一条」**，不是细节。
索引一行必须回答两个问题：① 我现在遇到的是不是这个 ② 该往哪个方向走。
细节等真要动它时再按 ID 去 archive 读 —— 那时读一条的成本，远低于每次启动读全部。

**三条配套硬规则**（写进 `worker-guide.md` 与各索引文件头部）：

1. 🔴 **索引里没写的细节不等于不存在** —— 不得凭索引一行就下「原文没提」的结论。
   这是本方案唯一的真实风险：把索引当全文用，会得出错误的「没有先例」判断。
2. 🔴 **新增条目：全文写进 archive，同时在索引补一行** —— 只写一边等于半失效。
3. 🔴 **`todo.md` 上限 250 行、单条 ≤8 行** —— 它是唯一会持续增长的索引，需要硬上限。

**归档方式：零删改。** 三个 archive 都是原文件全文原样搬入 + 一段头注，
`grep` 可证：decisions 67 条小节全在、troubleshooting 102 个不同 ID 全在、todo 全部小节全在。

**效果**：启动必读 **227,865 → 23,022 token（−90%）**。

**不做什么**：不删除任何历史条目、不合并相似条目、不按「已解决」筛掉 ——
判断哪条「以后用不上」本身就是会出错的判断，成本收益不对称（`[ITN-PREFIX-SHADOW-001]` 同型教训：
删除买不到收益，却可能制造静默回归）。
