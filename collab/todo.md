...
## 🔴 已拍板待派发 · HOTKEY-049 翻译热键与录音热键重复检测 + 拦截（2026-08-17 Gavin 拍板）

> **Gavin 原话**：「这个必须要做检测，如果重复要拦截提示用户重新设置」
>
> **时序**：排在 **BUILD-019 出包完成之后**派发，会另出 BUILD-020。
> 理由：BUILD-019 正在构建 `ui/`，此刻改前端会让产物与源码不一致
> （本轮已因 HOTKEY-048 暂停过一次构建，不重复踩）。

| 项 | 内容 |
| --- | --- |
| 编号 | **HOTKEY-049** |
| 文件域 | `ui/src/pages/HotkeySettings.tsx` + 三份 `ui/src/i18n/*.ts`（新增 key） |
| 负责人 | coder-1（047/048 同域，连续性最好） |
| 状态 | 🔜 等 BUILD-019 出完即派 |

### 缘起：文案已经这么写了，但没有任何代码在管

HOTKEY-048 把翻译热键页文案改成「推荐：左右 Ctrl / Alt，**不可和录音热键重复**」。
主控随即查证**三层强制机制全为空**：

| 层 | 实际情况 |
| --- | --- |
| `src-tauri/src/main.rs:45-47` `check_hotkey_available` | `vk_code` 落在 `0xA0..=0xA5`（左右 Ctrl/Alt/Shift）时**无条件 `return true`** —— 而这正是本文案推荐的键范围 |
| 同一函数的签名 | 只接 `vk_code` / `modifiers`，**拿不到当前配置**，结构上无法与语音热键比对 |
| `handleTranslationHotkeyKeyDown` | **零重复校验** |

→ 现状：把翻译热键设成与录音热键同一个键，程序照收，不拦不提示。**文案在撒谎。**

### 为什么必须拦（不是「用户自己注意」能兜住的）

录音与翻译是**两条独立注册路径**：录音走低层钩子 / `RegisterHotKey`；
...
现有冲突弹窗（`pendingHotkey`，`:415` 起）带 `t.hotkey_use_anyway`「仍然使用」按钮 ——
那是给**系统级占用**用的（用户可以强行覆盖）。

**本需求 Gavin 要的是「拦截 + 提示重新设置」，即不给放行选项。**
所以必须是**另一个弹窗/提示**，只有「知道了 / 重新设置」一个出口，
**不得**出现「仍然使用」。误复用即违背需求。

### 双向生效

- 设**翻译**热键时，与现有语音热键比对
- 设**语音**热键时，与现有翻译热键比对（**这一侧同样要做，别只做一边**）

### 新增 i18n（三语齐全，Gavin 强制要求所有界面文字走 i18n）

至少需要：冲突标题、说明文案（含「请重新设置」语义）、确认按钮。
`en.ts` / `zh-Hans.ts` / `zh-Hant.ts` **三份都要补**。
⚠️ 注意 `zh-Hant.ts` 另有历史缺 key 问题（见 `I18N-HANT-GAP-001`），别把两件事混在一起。

### 范围外（如实记录，不在本单做）

- **后端不做强制**：手改 `config.toml` 造成的重复**不受保护**。
  Gavin 说的是「设置时拦截」，属 UI 层。若要后端兜底需改
  `check_hotkey_available` 签名使其能读配置，属独立改动，另议。
- 不动 `TRANSLATION_SINGLE_KEYS` 候选集合
- 不动 `check_hotkey_available`

### 测试（阶段三，串行在代码之后）

`HotkeySettings.test.tsx` 已有 13 条基线，本单预计补：
判据 1 双向各 1 条、判据 2（若采纳）左右变体各 1 条、正常放行 1 条、
弹窗**无「仍然使用」出口** 1 条。
