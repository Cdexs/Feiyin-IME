...
| 任务 | Worker | 内容 | 状态 |
| --- | --- | --- | --- |
| `BUILD-165` | tester-1 | 出包：FIX-162 + FIX-164（✅ 六项核验全 PASS，sha `85c1cf26…` 异于 `b3043119…`，cargo test 全量 1110P/0F；修端测第 4/3/2 条；🔴 第 1 条托盘图标未修仅加日志） | ✅ **已交付，待主控验收/自测目视** |
| `FIX-162` | coder-2 | 回退 `WS_EX_COMPOSITED`（`main.rs:106` import + `:766` 扩展样式），🔴 明确不上备选方案 B（同样未实测） | ✅ **已验收通过**（`crash_reporter` 首轮 24F 定性为并行争用 `%APPDATA%`，**tester-1 全量回归未复现**=归档偶发）｜已进 BUILD-165 |
| `DIAG-163` | coder-1 | 端测 1/2/3 三问只读诊断，产出 `docs/DIAG-163-BUILD159-ENDTEST.md`；coder-1 已同意，执行顺序 Q3→Q1→Q2 | ✅ 已交付（Q3 机制定案/Q2 链路查通+阈值矛盾/Q1 机制未定需插桩，详见 logs） |
| `PROBE-INVENTORY-161` | coder-1 | 探针删除点位清单（coder-1 主动提出，主控已采纳） | ⏸️ **暂停**，端测优先 |
