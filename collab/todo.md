...
- **RC-B · 余数链不在语义边界终止**：「一块八**一斤**」后续量词短语首字「一」被吸入货币余数链 → `八一`=81 → `1块+81`=82。解释 #3 #4 #5
...
### 任务分解（待 Gavin 拍板 Q1-Q4 后派发）

| 编号 | 内容 | 负责人 | 状态 |
| --- | --- | --- | --- |
| ITN-FIX-CURRENCY-017 | RC-A/RC-B 修复（`src/itn.rs` + `itn-rules.toml`） | coder-1 | ⏸ 待拍板 |
| PROMPT-ARCH-A-018 | 分层契约结构落地，**逐字搬运、渲染 byte-identical**（`src/llm/mod.rs`） | coder-2 | ⏸ 待拍板 |
| PROMPT-ARCH-B-019 | T1-T4 契约测试（Topic 唯一归属 / 矛盾对 / 层序 / 预算） | tester-1 | ⏸ 待 A |
| PROMPT-ARCH-C-020 | L0 不变式 + 基座裁剪 + 删 OVERRIDE 补丁 | coder-2 | ⏸ 待 B |

**边界评估已做**：017 动 `src/itn.rs`/`itn-rules.toml`，018/020 动 `src/llm/mod.rs`/`src/i18n.rs`，**文件级零重叠**，可并行。
