...
| 编号 | 内容 | 负责人 | 状态 |
| --- | --- | --- | --- |
| ITN-FIX-CURRENCY-017 | RC-A（`两`消歧）/ RC-B（余数链边界终止）修复；`一斤二两`目标形态待 Worker 评估甲/乙 | coder-1 | 🔄 已 ACK，进行中 |
| PROMPT-ARCH-018 | A+C 合并：分层结构 + 元规则句 + L0 四条 + 基座裁剪（i18n **三处**）+ 删两处 OVERRIDE 补丁 | coder-2 | 🔄 已交付，验收中（741/1/6，唯一红=过时断言，真回归=0） |
| TEST-SYNC-019 | 017/018 断言同步 + **B 批契约测试**（T1 Topic 唯一归属 / T2 矛盾对 / T3 层序 / T4 长度预算） | tester-1 | ⏸ 阶段三，待 017+018 完成 |
