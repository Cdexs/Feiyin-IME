...
### OVERLAY-043 四项（主控已逐条核对代码，非采信 coder-2 汇报）

| # | 现象 | 核对结论 |
| --- | --- | --- |
| 1 | 波形动画未隐藏，文字被挤成窄条 | 属实。`main.rs:1786` 首行 `draw_recording_overlay` 整个重画，带进波形+分隔线+停止按钮 |
| 2 | 文字非白色 | 现象属实，**coder-2 给的原因是推测**（`:1816` 确设白色但在画完录音框之后）。**不采信，要求实测定位** |
| 3 | 右侧两个按钮（提交+停止） | 属实。说话中只需 stop；submit 属编辑态 |
| 4 | 窗口不随文字变宽、文字截断 | 属实但**比 coder-2 描述严重**：`main.rs:908-922`+`:940`，100ms 内不是「保持上次宽度」而是**用回默认 240px**，窗口在算出宽度与 240 之间来回跳 |
