...
## 🔄 进行中 · TEST-PROMPT-IND-034 IN-D 分叉点二分（2026-08-14 派发 tester-1）

> **来路**：033 收口后主控查看 IN-D 原始输出，发现一条此前从未被注意的线索。

**证据**（`ab033-results.json`，IN-D 在 G1/G2 共 6 次）：模型返回**输入的近乎逐字复制**，
口水词「啊」**6 次里 4 次原样保留**，`completion_tokens`=44~45 与输入等长。
对照同批 IN-B（`completion_tokens=77`）：口水词全删、句子改写、列表拆开。

🔴 **删口水词是无条件规则，不依赖枚举判断。模型连这个都没做**
→ 待判定命题：**IN-D 的失败不是「识别了枚举但没拆列表」，而是「整句被判定为无需处理」。**

**分量**：过去被证伪的 11 个假设**全是围绕「为什么没识别成列表」设计的**。
若分叉点在更早的「要不要处理这句话」，那些实验从一开始就找错了地方。

**核心测量维度变更**：`form`（bullet/para）退居次要，
新增 **`edited`**（输出是否逐字≠输入）与 **`filler_removed`**（口水词是否至少删掉一个）为主判据。

| 变体 | 改动 | 次数 | 隔离变量 |
| --- | --- | --- | --- |
| D0 | 原句 | 3 | 基线复现（须为 0/3，否则停手） |
| D1 | 删口水词「啊」 | 3 | **口水词是否触发器** |
| D2 | 删结尾句 | 3 | 收尾句是否压制处理 |
| D3 | 删引入句 | 3 | 引入句是否压制处理 |
| D4 | 三项改名词短语 | 3 | **动词性疑问短语假设** |
| CTRL | IN-B 原句 | 3 | 正向对照（须为 3/3，否则停手） |

共 18 次调用。提示词组装**完全复用 033 的 G1**（已验收 21863 B / −0.9%），不得改动。
**文件域**：仅 `collab/research/`，与 032（`src/itn.rs`）零重叠。
