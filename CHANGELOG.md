...
---
| FORMAT-F3-SHORTITEM-014 | 多行分支补「短项内联 / 长句列表」分流 + 四语措辞补充（Gavin 2026-08-02 端测：买菜清单被拆成四行 `- ` 列表，短名词短语应顿号内联）：`src/llm/mod.rs` build_output_format 真分支新增 F3-item form（DECISION RULE 后按项目形态分流：SHORT ≤6 字/词无谓语 → 内联分隔符，LONG → 列表；混合长短按多数项/不确定用列表；有序短项保留序号词）+ 分隔符表抽共享常量 INLINE_SEPARATOR_RULES 两分支共用（返回类型 &'static str → String）+ 四语补充（中文再者/最后一点/另外一点/第X条、English in addition/plus/and then/namely、日 ①②③/最初に/続いて/それに/加えて/ほかにも、韩 또/이어서/끝으로/아울러）+ 单行分支标记补齐至一致；llm:: 114/0 零红条 + 双 cargo check 0 errors + fmt 连带 3 处 | coder-2 | 2026-08-02 |
