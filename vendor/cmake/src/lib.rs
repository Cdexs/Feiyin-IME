...
impl Config {
    ...
    pub fn build(&mut self) -> PathBuf {
        ...
        if let Err(status) = try_run(&mut cmd, "cmake") {
            let cmake_target = self
                .cmake_target
                .clone()
                .unwrap_or_else(|| "install".to_string());

            let fallback_install = cfg!(windows)
                && !self.no_build_target
                && cmake_target.eq_ignore_ascii_case("install");
            ...
            let mut install_cmd = self.cmake_build_command(&target);
            ...
            install_cmd
                .arg(format!("-DBUILD_TYPE={}", profile))
                .arg("-P")
                .arg("cmake_install.cmake");

            match try_run(&mut install_cmd, "cmake") {
                Ok(_) => {}
                Err(install_status) => {
                    fail(&format!(
                        "cmake --build install returned {}, and fallback cmake_install.cmake returned {}",
...
fn run(cmd: &mut Command, program: &str) {
    if let Err(status) = try_run(cmd, program) {
...
fn try_run(cmd: &mut Command, program: &str) -> Result<ExitStatus, ExitStatus> {
